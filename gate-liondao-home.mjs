...
ok('v2.3: supply map, lore and rarity open coming.html?tile=<id> (slots with a name) and read muted; mint and alliance are real pages, not muted', [1, 2, 3].every(i => /coming\.html\?tile=/.test(nav[i].getAttribute('onclick')) && nav[i].classList.contains('ht-soon')) && [4, 6].every(i => !/coming\.html/.test(nav[i].getAttribute('onclick')) && !nav[i].classList.contains('ht-soon')), [1, 2, 3, 4, 6].map(i => nav[i].getAttribute('onclick')));
...
ok('ROAR bar: total, staked and burned are Unknown in this run (chain reads blocked) — NO segment is drawn and no 100 % appears, because a part against an unread total is a phantom (1.2.0)', T(bars[0].parentElement).split('Coming').length - 1 >= 3 && !T(bars[0].nextElementSibling).includes('100.0%') && bars[0].querySelectorAll('span').length === 0, [T(bars[0].nextElementSibling), bars[0].querySelectorAll('span').length]);
...
ok('the Rarity tile is a SLOT again (owner 2026-09-22: a placeholder for the pride\'s own rarity page, not the explorer); Lore and the supply map stay muted slots — three in all', [...nav].some(t => T(t).startsWith('Rarity') && /coming\.html\?tile=rarity/.test(t.getAttribute('onclick')) && t.classList.contains('ht-soon')) && ![...nav].some(t => /nft-explorer-index/.test(t.getAttribute('onclick') || '')) && [...nav].filter(t => t.classList.contains('ht-soon')).length === 3, [...nav].map(t => T(t).slice(0, 12) + '→' + (t.getAttribute('onclick') || '').slice(14, 50)));
