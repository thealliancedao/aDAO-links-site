...
const tenants0 = J(path.join(CORE, 'docs/curated/tenants.json'));
...
// 1.2.0 SHAPE fixtures (marked values, cosmos-sdk / solana-rpc schemas): the pyROAR cw20 answers token_info + balances; the Solana host answers getTokenSupply + getTokenLargestAccounts
const PY = tenants0.tenants.liondao.burn.pyroar_cw20, PY_RECEIVER = tenants0.tenants.liondao.burn.festival_receiver, PY_TREASURY = Object.keys(tenants0.tenants.liondao.wallets).find(a => tenants0.tenants.liondao.wallets[a].counts_as === 'treasury');
const PY_FIX = { total: '109690675868390000', treasury: '6240000000000', receiver: '100000000000000' };   // 109.69B · 6.24M · 100M (pyROAR, 6 dp)
const SOL_FIX = { supply: 1000000000, largest: [['SoLpooLvault111', 500000000]].concat(Array.from({ length: 19 }, (_, k) => ['SoLacct' + (k + 2), (300000000 / 19)])) };
const jsonOk = (v) => Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(v), text: () => Promise.resolve(JSON.stringify(v)) });
const solRpc = (u, o) => { if (!/api\.mainnet-beta\.solana\.com/.test(u) || !o || o.method !== 'POST') return null; const b = JSON.parse(o.body); if (b.method === 'getTokenSupply') return jsonOk({ jsonrpc: '2.0', id: 1, result: { context: { slot: 1 }, value: { amount: String(SOL_FIX.supply * 1e6), decimals: 6, uiAmount: SOL_FIX.supply, uiAmountString: String(SOL_FIX.supply) } } }); if (b.method === 'getTokenLargestAccounts') return jsonOk({ jsonrpc: '2.0', id: 1, result: { context: { slot: 1 }, value: SOL_FIX.largest.map(([a, n]) => ({ address: a, amount: String(Math.round(n * 1e6)), decimals: 6, uiAmount: n, uiAmountString: String(n) })) } }); return jsonOk({ jsonrpc: '2.0', id: 1, error: { code: -32601, message: 'method not found' } }); };
const pyRoar = (u) => { const m = u.match(/\/cosmwasm\/wasm\/v1\/contract\/([a-z0-9]+)\/smart\/([A-Za-z0-9+/=]+)/); if (!m || m[1] !== PY) return null; const q = JSON.parse(Buffer.from(m[2], 'base64').toString()); if (q.token_info) return jsonOk({ data: { name: 'Burnt ROAR', symbol: 'pyROAR', decimals: 6, total_supply: PY_FIX.total } }); if (q.balance) { const a = q.balance.address; return jsonOk({ data: { balance: a === PY_TREASURY ? PY_FIX.treasury : a === PY_RECEIVER ? PY_FIX.receiver : '0' } }); } return null; };
...
const dom = new JSDOM(html, { url: 'https://thealliancedao.com/liondao/', runScripts: 'dangerously', pretendToBeVisual: true, beforeParse(win) {
  ...
  win.fetch = (u, o) => { fetched.push(String(u)); const rpc = solRpc(String(u), o); if (rpc) return rpc; const py = pyRoar(String(u)); if (py) return py; const f = files(u); if (f && fs.existsSync(f)) { const t = fs.readFileSync(f, 'utf8'); return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(JSON.parse(t)), text: () => Promise.resolve(t) }); } return Promise.resolve({ ok: false, status: 404, json: () => Promise.reject(new Error('404')), text: () => Promise.resolve('') }); };
...
ok('page mounted the engine (home-tiles ' + (w.HomeTiles && w.HomeTiles.VERSION) + ')', !!d.querySelector('.ht') && w.HomeTiles.VERSION === '1.5.1');
...
const hero = d.querySelector('.ht-hero'); ok('hero (1.2.1, "more Lion DAO"): "The pride" + LFR badge, two mane rings of rays and a medallion of the tenant\'s own logo from the registry — no caution tape', hero && T(hero).startsWith('The pride') && T(hero.querySelector('.ht-lfr')) === 'LFR' && hero.classList.contains('ht-hero-mane') && hero.querySelectorAll('.ht-mane').length === 2 && hero.querySelector('.ht-medal img') && hero.querySelector('.ht-medal img').getAttribute('src') === LD.logo && !/repeating-linear-gradient/.test(w.HomeTiles.CSS.join ? w.HomeTiles.CSS.join('') : String(w.HomeTiles.CSS)), T(hero).slice(0, 80));
...
const bars = d.querySelectorAll('.ht-sec .ht-bar'); ok('four bars: ROAR · pixeLions · pyROAR · ROAR20 (1.2.0), in a 2-column grid', bars.length === 4 && bars[0].closest('.ht-grid').classList.contains('ht-g2'), bars.length);
{ const py = bars[2].parentElement, leg = T(bars[2].nextElementSibling); const tot = 109690675868.39, ro = 6240000, rc = 100000000;
  ok('pyROAR bar (no holder product in this gate → the live fallback): total 109.69B live from token_info; roster 6.24M + receiver 100M drawn; contracts / top-10 / other wallets read Coming with the product reason; black remainder', T(py).includes('of 109.69B') && leg.includes('Lion DAO roster ' + w.HomeTiles.fmt.big(ro)) && leg.includes('Festival receiver (Enterprise core) ' + w.HomeTiles.fmt.big(rc)) && bars[2].querySelectorAll('span[title]:not([title="unattributed / unknown"])').length === 2 && !!bars[2].querySelector('span[title="unattributed / unknown"]') && [...py.querySelectorAll('.ht-leg .ht-unk')].length === 3 && [...py.querySelectorAll('.ht-leg .ht-unk')].every(e => /holder product not read/.test(e.getAttribute('title'))), [T(py).slice(0, 120), leg]);
  ok("pyROAR note is chain facts only — no published figure, no post: no '224', no '107B', no 'reported', no 'theirs'; it links to the burn leaderboard page", !/224|107B|reported|theirs|tweet/.test(T(py)) && !!py.querySelector('a[href="/liondao/burn.html"]'), T(py).slice(-300));
  const r2 = bars[3].parentElement, leg2 = T(bars[3].nextElementSibling);
  ok('ROAR20 bar (no holder product → fallback): supply 1B live from getTokenSupply; the three product parts read Coming (the live read shows only the 20 largest accounts) and the only segment is the black unattributed remainder; the note carries the live fact — largest account 50%', T(r2).includes('of 1B') && [...r2.querySelectorAll('.ht-leg .ht-unk')].length === 3 && bars[3].querySelectorAll('span').length === 1 && bars[3].querySelector('span').getAttribute('title') === 'unattributed / unknown' && /largest token account holds 50% of supply/.test(T(r2)) && !!r2.querySelector('a[href="/liondao/roar20.html"]'), [T(r2).includes('of 1B'), [...r2.querySelectorAll('.ht-leg .ht-unk')].length, bars[3].querySelectorAll('span').length, /largest token account holds 50% of supply/.test(T(r2)), !!r2.querySelector('a[href="/liondao/roar20.html"]')]);
  const D0 = w.HomeTiles._M.D; ok('derive (live fallback): roar20.rest = supply − top1 − top2_20 = 200M; pyroar.burners_remainder = total − roster − receiver; held_roster_pct ≈ 0.0057%; no product objects', Math.abs(D0.roar20.rest - 200000000) < 1 && Math.abs(D0.pyroar.burners_remainder - (tot - ro - rc)) < 1 && Math.abs(D0.pyroar.held_roster_pct - ro / tot * 100) < 1e-9 && D0.pyroar.product === null && D0.roar20.product === null, [D0.roar20.rest, D0.pyroar.burners_remainder, D0.pyroar.held_roster_pct]); }
...
ok('ROAR bar (1.3.1): the total is 1T minted from the registry even with the chain dead; burned / staked / treasury / pools all read Coming (no reads) — the only segment is the black remainder, no 100 %', T(bars[0].parentElement).includes('of 1T') && T(bars[0].parentElement).split('Coming').length - 1 >= 4 && !T(bars[0].nextElementSibling).includes('100.0%') && bars[0].querySelectorAll('span[title]:not([title="unattributed / unknown"])').length === 0, [T(bars[0].nextElementSibling), bars[0].querySelectorAll('span').length]);

{ const D1 = w.HomeTiles.derive(Object.assign({}, w.HomeTiles._M.S, { positions: null }), w.HomeTiles._M.cfg, w.HomeTiles._M.ctx); ok('derive keeps a known treasury when the total is unread — the RENDERER, not derive, withholds the segment (pixeLions bar with a total still draws its parts)', bars[1].querySelectorAll('span').length >= 4, bars[1].querySelectorAll('span').length); }
...
ok('Validator group Coming (LCD blocked) · pixeLions rewards Coming (no positions product in this gate — the reason says so)', rewTiles[1].querySelector('.ht-v .ht-unk') && rewTiles[2].querySelector('.ht-v .ht-unk') && /distributor/.test(rewTiles[2].querySelector('.ht-v .ht-unk').getAttribute('title')));
...
ok('Staking rewards (no positions product in this gate): the tile reads Coming with the reason; the source names the rate and the two yield bases; never the old 5B arithmetic', plTiles[3].querySelector('.ht-v .ht-unk') && aprT.startsWith('Staking rewards') && /the rate is the contract/.test(aprT) && !aprT.includes('5B'), aprT);

console.log('— Burning Lions row');
ok('Burning Lions: "12 planned" from the REGISTRY, labeled (the cw721 read is dead in this gate); top burner Coming (holder product not read); the contract is on file', T(rows[2].querySelectorAll('.ht-tile')[0]).includes('Supply12 planned') && /cw721 not read/.test(T(rows[2].querySelectorAll('.ht-tile')[0])) && rows[2].querySelectorAll('.ht-tile')[2].querySelector('.ht-v .ht-unk') && LD.burning_lions.contract === 'terra1cfk54jzu6wsr7c7eqhvs3znkkuxc7pvr7awdvheg7szvnlmduvus95a0q0' && LD.burning_lions.supply === 12, T(rows[2]).slice(0, 200));

console.log('— burn + ROAR20 (registry facts, source-labeled)');
ok('Burning Lions row: festival burn = reported community + DAO (' + w.HomeTiles.fmt.big(LD.burn.reported.community_burned + LD.burn.reported.dao_burned) + ') labeled as Lion DAO\'s, with the measured figure Coming (chip)', T(rows[2]).includes(w.HomeTiles.fmt.big(LD.burn.reported.community_burned + LD.burn.reported.dao_burned)) && T(rows[2]).includes('reported by Lion DAO') && /measured: Coming gone from supply/.test(T(rows[2])), T(rows[2]).slice(0, 300));
...
console.log('— marketplaces (1.2.0: aDAO\'s shape — three venue cards · Live Activity · All Current Listings grid · Top 10 behind a button)');
const bl = nap.token_prices.bLUNA.final_price_usd; const bblMin = S.marketplaces.bbl.by_token.bLUNA.min;
const mk = d.getElementById('ht-market'); const venues = [...mk.querySelectorAll('.ht-venue')];
ok('three venue cards in aDAO\'s order: BBL · Boost · Atrium, each with a View link into the venue for THIS collection', venues.length === 3 && venues.map(v => T(v.querySelector('.ht-venue-h b'))).join('|') === 'BBL Marketplace|Boost Marketplace|Atrium Marketplace' && venues[0].querySelector('a.ht-view').getAttribute('href').includes('backbonelabs.io') && venues[0].querySelector('a.ht-view').getAttribute('href').includes(LD ? '' : '') && venues[2].querySelector('a.ht-view').getAttribute('href').includes('atrium.markets'), venues.map(v => T(v.querySelector('.ht-venue-h')) + ' ' + (v.querySelector('a.ht-view') || {}).getAttribute?.('href')));
ok('collection tabs in the header: pixeLions active, Burning Lions muted with its reason', [...mk.querySelectorAll('.ht-ctab')].some(t => T(t) === 'pixeLions' && t.classList.contains('ht-on')) && [...mk.querySelectorAll('.ht-ctab')].some(t => T(t) === 'Burning Lions' && t.classList.contains('ht-off') && /not onboarded/.test(t.getAttribute('title'))));
const rowOf = (card, k) => [...card.querySelectorAll('.ht-mrow')].find(r => T(r.querySelector('.ht-mk-k')) === k);
ok(`BBL Floor Price = ${bblMin} bLUNA (${w.HomeTiles.fmt.usd(bblMin * bl, 2)}) · Currently Listed ${S.bbl_listed_count} · rows in aDAO's order`, T(rowOf(venues[0], 'Floor Price')).includes(bblMin + ' bLUNA') && T(rowOf(venues[0], 'Floor Price')).includes(w.HomeTiles.fmt.usd(bblMin * bl, 2)) && T(rowOf(venues[0], 'Currently Listed')).includes(String(S.bbl_listed_count)) && [...venues[0].querySelectorAll('.ht-mk-k')].map(T).slice(0, 4).join('|') === 'Floor Price|All-Time Volume|Total Sales|Currently Listed', [...venues[0].querySelectorAll('.ht-mrow')].map(T));
const SE = J(path.join(NFTC, 'pixel-lions/snapshots/sales-enriched.json')); const bblSales = SE.sales.filter(x => /^bbl/i.test(x.marketplace)); const bblUsd = bblSales.reduce((t, x) => t + (x.notional_usd || 0), 0);
ok(`BBL All-Time Volume = our ledger's BBL sales ${usd(bblUsd)} at sale time · Total Sales ${bblSales.length} (sales-enriched folded per venue)`, T(rowOf(venues[0], 'All-Time Volume')).includes(usd(bblUsd)) && T(rowOf(venues[0], 'All-Time Volume')).includes('at sale time') && T(rowOf(venues[0], 'Total Sales')).includes(bblSales.length.toLocaleString() + ' sales'), [T(rowOf(venues[0], 'All-Time Volume')), T(rowOf(venues[0], 'Total Sales'))]);
const B = J(path.join(NFTC, 'pixel-lions/snapshots/explorer-bundle.json')); const li = B.fields.indexOf('listing_usd'), ri = B.fields.indexOf('intended_rank'), fi = B.fields.indexOf('flags'); const listed = B.rows.filter(r => typeof r[li] === 'number');
const bblBase = listed.filter(r => (r[fi] & B.flagBits.bbl_listed) && r[ri] !== 1); const bblBaseMin = Math.min(...bblBase.map(r => r[li]));
ok(`BBL class floors: Base FP from ${w.HomeTiles.fmt.usd(bblBaseMin, 2)} · ${bblBase.length} listed; 1 of 1 FP N/A when none; Burning Lions FP N/A with its reason`, T(rowOf(venues[0], 'Base FP')).includes('from ' + w.HomeTiles.fmt.usd(bblBaseMin, 2)) && T(rowOf(venues[0], 'Base FP')).includes(bblBase.length + ' listed') && !!rowOf(venues[0], '1 of 1 FP') && T(rowOf(venues[0], 'Burning Lions FP')).includes('N/A'), [T(rowOf(venues[0], 'Base FP')), T(rowOf(venues[0], '1 of 1 FP')), T(rowOf(venues[0], 'Burning Lions FP'))]);
ok('Atrium: 2 listed in SOLID (a token the feed prices) — the floor is a SOLID amount with USD, never a blank', T(rowOf(venues[2], 'Floor Price')).includes('SOLID') && T(rowOf(venues[2], 'Currently Listed')).includes('2'), T(rowOf(venues[2], 'Floor Price')));
// ---- listings grid
const listEl = d.getElementById('ht-list'); const cards = () => [...d.querySelectorAll('#ht-list .ht-lcard')];
ok(`All Current Listings: (12 of ${listed.length}) · a 3-column card grid · 12 cards low → high · first = ${usd(Math.min(...listed.map(r => r[li])))}`, T(d.getElementById('ht-list-count')) === '(12 of ' + listed.length + ')' && listEl.classList.contains('ht-lgrid') && cards().length === 12 && T(cards()[0].querySelector('.ht-lc-p b')) === w.HomeTiles.fmt.usd(Math.min(...listed.map(r => r[li])), 2), [T(d.getElementById('ht-list-count')), cards().length]);
ok('a card reads: "pixeLion #<id>" + Lion DAO chip · class · Rank #N · venue · USD · "listed in <denom>"; it links into the explorer with the tenant kept', /^pixeLion #\d+/.test(T(cards()[0].querySelector('.ht-lc-n'))) && T(cards()[0].querySelector('.ht-chip')) === 'Lion DAO' && /Rank #/.test(T(cards()[0].querySelector('.ht-lc-s'))) && /listed in (bLUNA|SOLID|USDC\.n)/.test(T(cards()[0].querySelector('.ht-lc-p small'))) && cards()[0].getAttribute('href').startsWith('/nft-explorer-index.html?tenant=liondao&search='), T(cards()[0]));
ok('filters: Marketplace (All · BBL · Boost · Atrium with counts) and Class (All · Base · 1 of 1 · Burning Lions disabled) + the sort button', ['All', 'BBL', 'Boost', 'Atrium', 'Base', '1 of 1'].every(t => [...d.querySelectorAll('#ht-list-ctl .ht-tab')].some(b => T(b).startsWith(t))) && [...d.querySelectorAll('#ht-list-ctl .ht-tab')].some(b => T(b) === 'Burning Lions' && b.disabled) && /USD low → high/.test(T(d.getElementById('ht-list-ctl'))), T(d.getElementById('ht-list-ctl')));
ok(`"Load 12 more (${listed.length - 12} left)"`, T(d.getElementById('ht-list-more')) === 'Load 12 more (' + (listed.length - 12) + ' left)', T(d.getElementById('ht-list-more')));
w.HomeTiles.setList('venue', 'atrium'); const atriumN = listed.filter(r => r[fi] & B.flagBits.atrium_listed).length;
ok(`venue filter Atrium → ${atriumN} cards, count "(${atriumN} of ${atriumN})"`, cards().length === atriumN && T(d.getElementById('ht-list-count')) === '(' + atriumN + ' of ' + atriumN + ')', [cards().length, T(d.getElementById('ht-list-count'))]);
w.HomeTiles.setList('venue', 'all'); w.HomeTiles.setList('sort', 'desc'); ok('USD high → low re-sorts (first card = the dearest listing)', T(cards()[0].querySelector('.ht-lc-p b')) === w.HomeTiles.fmt.usd(Math.max(...listed.map(r => r[li])), 2) && /high → low/.test(T(d.getElementById('ht-list-ctl'))));
w.HomeTiles.setList('sort', 'asc'); w.HomeTiles.setList('class', 'rank1'); const r1 = listed.filter(r => r[ri] === 1).length; ok(`class "1 of 1" (rank 1) filter → ${r1} cards`, cards().length === r1 || (r1 === 0 && T(listEl).includes('Nothing listed')), cards().length);
w.HomeTiles.setList('class', 'all');
// ---- top 10 behind a button
const salesEl = d.getElementById('ht-sales');
ok('Top 10 All-Time Sales is a button; the list is hidden until pressed', /Top 10 All-Time Sales/.test(T(mk.querySelector('.ht-btn'))) && salesEl.classList.contains('ht-hidden'));
w.HomeTiles.toggleSales(); await new Promise(r => setTimeout(r, 300));
const top = SE.sales.filter(x => typeof x.notional_usd === 'number').sort((a, b) => b.notional_usd - a.notional_usd)[0]; const srows = [...salesEl.querySelectorAll('.ht-row')];
ok(`pressed: 10 rows, #1 = pixeLion #${top.token_id} ${usd(top.notional_usd)} (USD at sale) with "now" beside it`, !salesEl.classList.contains('ht-hidden') && srows.length === 10 && T(srows[0]).includes('#1 pixeLion #' + top.token_id) && T(srows[0].querySelector('.ht-rv')).startsWith(usd(top.notional_usd)) && T(srows[0]).includes('now '), [srows.length, T(srows[0] || null)]);
...
// ---- live activity
const la = d.getElementById('ht-act'); const larows = la.querySelectorAll('.la-row'); const kinds = d.getElementById('ht-act-kinds');
ok('live activity renders through lib/live-activity.js, full width, in a scrolling box (rows = what the ledger holds in the window, own tenant = liondao, deal filter on)', larows.length > 0 && larows.length <= 12 && la.classList.contains('ht-scroll') && !w.HomeTiles._state.actShowAll && T(kinds).includes('Deal filter on'), [larows.length, T(d.getElementById('ht-act-count'))]);
ok('window chips 24h · 7d · 30d, feed chips All · aDAO · Lion DAO · TLA Locks, a reload button', ['24h', '7d', '30d', 'aDAO', 'Lion DAO', 'TLA Locks'].every(t => T(d.getElementById('ht-act-ctl')).includes(t)) && !!d.querySelector('#ht-act-ctl button[title="reload the feed"]'), T(d.getElementById('ht-act-ctl')));
ok("kind chips as on aDAO (Sales · Listings · Price changes · Delistings · Bids · Stakes · Unstakes · Breaks · Transfers · Locks · Lock housekeeping) with counts; housekeeping OFF by default (quiet, never featured); 'None' when every chip is on", ['Sales', 'Listings', 'Price changes', 'Delistings', 'Bids', 'Stakes', 'Unstakes', 'Breaks', 'Transfers', 'Locks', 'Lock housekeeping'].every(t => [...kinds.querySelectorAll('.ht-tab')].some(b => T(b).startsWith(t))) && ![...kinds.querySelectorAll('.ht-tab')].find(b => T(b).startsWith('Lock housekeeping')).classList.contains('ht-on') && [...kinds.querySelectorAll('.ht-tab')].find(b => T(b).startsWith('Locks')).classList.contains('ht-on'), T(kinds));
{ const before = la.querySelectorAll('.la-row').length; w.HomeTiles.setAct('kinds', false); const none = la.querySelectorAll('.la-row').length; w.HomeTiles.setAct('kind', 'locks'); const locksOnly = la.querySelectorAll('.la-row').length; w.HomeTiles.setAct('kinds', true); w.HomeTiles.setAct('kind', 'housekeeping');
  ok(`kind chips filter the rows: all off → ${none} (featured rows only) · Locks alone → ${locksOnly} · back on → ${la.querySelectorAll('.la-row').length}`, none <= before && locksOnly >= none && la.querySelectorAll('.la-row').length === before, [before, none, locksOnly]); }
...
ok('the Rarity tile is a SLOT (owner 2026-09-22: a placeholder for the pride\'s own rarity page, not the explorer); Lore too — two in all, the supply map is a page now', [...nav].some(t => T(t).startsWith('Rarity') && /coming\.html\?tile=rarity/.test(t.getAttribute('onclick')) && t.classList.contains('ht-soon')) && ![...nav].some(t => /nft-explorer-index/.test(t.getAttribute('onclick') || '')) && [...nav].filter(t => t.classList.contains('ht-soon')).length === 2, [...nav].map(t => T(t).slice(0, 12) + '→' + (t.getAttribute('onclick') || '').slice(14, 50)));
ok('the chip says "Coming" (owner), with the reason still on hover', [...d.querySelectorAll('.ht-unk')].length > 0 && [...d.querySelectorAll('.ht-unk')].every(e => T(e) === 'Coming' && e.getAttribute('title')), [...d.querySelectorAll('.ht-unk')].slice(0, 3).map(e => T(e) + ' · ' + e.getAttribute('title')));
{ const S1 = Object.assign({}, w.HomeTiles._M.S); delete S1.sol_supply; delete S1.sol_largest; delete S1.lcd_pyroar_info; const D1 = w.HomeTiles.derive(S1, w.HomeTiles._M.cfg, w.HomeTiles._M.ctx);
  ok('registry fallback (1.2.1): with the Solana RPC and the pyROAR token_info unread, the totals come from tenants.json — ROAR20 1B "registry (minted; burns lower it)", pyROAR 109.69B "registry, measured 2026-09-21" — and the parts stay Coming (no segment drawn)', D1.roar20.supply === 1e9 && D1.roar20.supply_source === 'registry (minted; burns lower it)' && Math.abs(D1.pyroar.total_supply - 109690675868.39) < 1 && D1.pyroar.total_source === 'registry, measured 2026-09-21' && D1.roar20.p_program == null, [D1.roar20.supply, D1.roar20.supply_source, D1.pyroar.total_supply, D1.pyroar.total_source]);
  ok('with the reads answering, the totals are live (this run)', w.HomeTiles._M.D.pyroar.total_source === 'live' && w.HomeTiles._M.D.roar20.supply_source === 'Solana RPC getTokenSupply', [w.HomeTiles._M.D.pyroar.total_source, w.HomeTiles._M.D.roar20.supply_source]); }
