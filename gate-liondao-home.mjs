...
ok('page mounted the engine (home-tiles ' + (w.HomeTiles && w.HomeTiles.VERSION) + ')', !!d.querySelector('.ht') && w.HomeTiles.VERSION === '1.2.1');
...
{ const py = bars[2].parentElement, leg = T(bars[2].nextElementSibling); const tot = 109690675868.39, ro = 6240000, rc = 100000000;
  ok('pyROAR bar: total 109.69B live from token_info; roster 6.24M and receiver 100M drawn; the black remainder = the burners (' + w.HomeTiles.fmt.big(tot - ro - rc) + ')', T(py).includes('of 109.69B') && leg.includes('Lion DAO roster ' + w.HomeTiles.fmt.big(ro)) && leg.includes('Festival receiver (Enterprise core) ' + w.HomeTiles.fmt.big(rc)) && bars[2].querySelectorAll('span[title]:not([title="unattributed / unknown"])').length === 2 && !!bars[2].querySelector('span[title="unattributed / unknown"]'), [T(py).slice(0, 120), leg]);
  ok("pyROAR note carries Lion DAO's published figures as theirs (224 wallets · 10B community + 107B DAO · top 1.4B)", T(py).includes('224') && T(py).includes('10B') && T(py).includes('107B') && T(py).includes('1.4B') && /theirs/.test(T(py)), T(py).slice(-260));
  const r2 = bars[3].parentElement, leg2 = T(bars[3].nextElementSibling);
  ok('ROAR20 bar: supply 1B from getTokenSupply; largest account 500M (50.0%) · accounts 2–20 300M (30.0%) · remainder 20% black', T(r2).includes('of 1B') && leg2.includes('Largest token account 500M') && leg2.includes('50.0%') && leg2.includes('Accounts 2–20 300M') && leg2.includes('30.0%') && !!bars[3].querySelector('span[title="unattributed / unknown"]'), [T(r2).slice(0, 100), leg2]);
  ok('ROAR20 note names the largest account (short) and says token accounts ≠ wallets, holders need an indexer', T(r2).includes('SoLpooLvau') && /not wallets/.test(T(r2)) && /indexer/.test(T(r2)), T(r2).slice(-300));
  const D0 = w.HomeTiles._M.D; ok('derive: roar20.rest = supply − top1 − top2_20 = 200M; pyroar.burners_remainder = total − roster − receiver; held_roster_pct ≈ 0.0057%', Math.abs(D0.roar20.rest - 200000000) < 1 && Math.abs(D0.pyroar.burners_remainder - (tot - ro - rc)) < 1 && Math.abs(D0.pyroar.held_roster_pct - ro / tot * 100) < 1e-9, [D0.roar20.rest, D0.pyroar.burners_remainder, D0.pyroar.held_roster_pct]); }
