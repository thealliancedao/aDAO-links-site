...
ok('page mounted the engine (home-tiles ' + (w.HomeTiles && w.HomeTiles.VERSION) + ')', !!d.querySelector('.ht') && w.HomeTiles.VERSION === '1.2.0');
...
ok('ROAR bar: total, staked and burned are Unknown in this run (chain reads blocked) — NO segment is drawn and no 100 % appears, because a part against an unread total is a phantom (1.2.0)', T(bars[0].parentElement).split('Unknown').length - 1 >= 3 && !T(bars[0].nextElementSibling).includes('100.0%') && bars[0].querySelectorAll('span').length === 0, [T(bars[0].nextElementSibling), bars[0].querySelectorAll('span').length]);
