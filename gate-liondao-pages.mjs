...
{ const req = createRequire(import.meta.url); const LD = req(path.join(SITE, 'liondao/lib/ld.js'));
  ok('ld.js 1.1.0 loads in node; fmt.usd / usdFull / big / price agree with the page expectations', LD.VERSION === '1.1.0' && LD.fmt.usd(188105.3) === '$188,105' && LD.fmt.big(298564000000) === '298.56B' && LD.fmt.price(3.21e-7) === '$0.0₆321', [LD.fmt.usd(188105.3), LD.fmt.big(298564000000), LD.fmt.price(3.21e-7)]);
...
{ const { d, T, fetched } = await run('liondao/index.html', 'https://thealliancedao.com/liondao/', { lcd: (u) => (/publicnode|coingecko|dexscreener/.test(u) ? null : undefined), wait: 5000 });
  ...
  ok('page rev 2.4 in the footer', has(T(d.getElementById('page-rev')), 'Rev 2.8'));
