...
{ const { d, T } = await run('liondao/dao_unclaimed.html', 'https://thealliancedao.com/liondao/dao_unclaimed.html');
  ...
  { const rt = T(d.getElementById('rates')); const d1 = PRf.distributions[0];
    ok('"What staking pays": one row per distribution — ROAR ' + fmtBig(d1.emission_per_year) + ' / yr, per lion, USD per lion; DROGO row USD reads Coming (not in the feed); APR at floor ' + PRf.apr.apr_pct_at_floor.toFixed(1) + '%', has(rt, fmtBig(d1.emission_per_year) + ' / yr') && has(rt, LDN.fmt.usd(d1.per_token_usd_per_year, 2)) && d.querySelectorAll('#rates tbody tr').length === 2 && d.querySelector('#rates tbody tr:nth-child(2) .ld-unk') && has(rt, 'APR at floor') && has(rt, LDN.fmt.pctPlain(PRf.apr.apr_pct_at_floor, 1)) && has(rt, PRf.staked_count.toLocaleString() + ' pixeLions staked'), rt.slice(0, 500)); }
...
{ const { d, T, fetched } = await run('liondao/index.html', 'https://thealliancedao.com/liondao/', { lcd: (u) => (/publicnode|coingecko|dexscreener/.test(u) ? null : undefined), wait: 5000 });
  ...
  ok('page rev 2.4 in the footer', has(T(d.getElementById('page-rev')), 'Rev 2.5'));
