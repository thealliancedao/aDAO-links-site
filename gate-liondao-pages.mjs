...
{ const req = createRequire(import.meta.url); const LD = req(path.join(SITE, 'liondao/lib/ld.js'));
  ok('ld.js 1.0.2 loads in node; fmt.usd / usdFull / big / price agree with the page expectations', LD.VERSION === '1.0.2' && LD.fmt.usd(188105.3) === '$188,105' && LD.fmt.big(298564000000) === '298.56B' && LD.fmt.price(3.21e-7) === '$0.0₆321', [LD.fmt.usd(188105.3), LD.fmt.big(298564000000), LD.fmt.price(3.21e-7)]);
...
{ const { d, T, fetched } = await run('liondao/index.html', 'https://thealliancedao.com/liondao/', { lcd: (u) => (/publicnode|coingecko|dexscreener/.test(u) ? null : undefined), wait: 5000 });
  ...
  ok('the big tile opens ecosystem.html; supply/lore/rarity open coming.html?tile=… muted (slots with a name); mint → mint.html; alliance → alliance.html — nothing else is a slot', (() => { const tiles = Array.from(home.querySelectorAll('.ht-nav .ht-tile')); const eco = tiles[0]; return /ecosystem\.html/.test(eco.getAttribute('onclick')) && tiles.filter(t => /coming\.html\?tile=/.test(t.getAttribute('onclick')) && t.classList.contains('ht-soon')).length === 3 && !tiles.some(t => /nft-explorer-index/.test(t.getAttribute('onclick') || '')) && tiles.some(t => /\/liondao\/mint\.html/.test(t.getAttribute('onclick')) && !t.classList.contains('ht-soon')) && tiles.some(t => /\/liondao\/alliance\.html/.test(t.getAttribute('onclick')) && !t.classList.contains('ht-soon')); })(), Array.from(home.querySelectorAll('.ht-nav .ht-tile')).map(t => (t.getAttribute('onclick') || '').slice(14, 60) + (t.classList.contains('ht-soon') ? ' [soon]' : '')));
  ok('DAO links menu carries the eight pages (treasury, TLA, unclaimed, validator, mint, alliance, burn, roar20)', ['/liondao/dao_treasury.html', '/liondao/dao_tla_deposits.html', '/liondao/dao_unclaimed.html', '/liondao/validator.html', '/liondao/mint.html', '/liondao/alliance.html', '/liondao/burn.html', '/liondao/roar20.html'].every(h => home.querySelector('.ht-menu a[href="' + h + '"]')));
  ...
  ok('page rev 2.4 in the footer', has(T(d.getElementById('page-rev')), 'Rev 2.7'));
  { const PH = J(path.join(DAOO, 'lion-dao/burn/holders.json')); const RH = J(path.join(DAOO, 'lion-dao/roar20/holders.json')); const bars = home.querySelectorAll('.ht-bar'); const py = bars[2].parentElement, r20 = bars[3].parentElement; const Tp = T(py), Tr = T(r20);
    ...
    { const d1 = cur.pl_rewards.distributions.find(x => x.symbol === 'ROAR'); const tile = Array.from(home.querySelectorAll('.ht-tile')).find(x => T(x).startsWith('Staking rewards')); const tt = T(tile);
      ok('home "Staking rewards" tile = ' + (d1.per_token_per_year / 8760).toFixed(1) + ' ROAR/hr per staked lion (the rate), a day / a year / USD a year under it, DROGO named as unpriced — no APR, no floor', !!tile && has(tt, LDN.fmt.num(d1.per_token_per_year / 8760) + ' ROAR/hr') && has(tt, 'a day') && has(tt, 'DROGO/hour (unpriced)') && !/APR|floor/i.test(tt), tt); } } }
