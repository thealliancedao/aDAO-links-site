...
{ const PH = J(path.join(DAOO, 'lion-dao/burn/holders.json')); const RH = J(path.join(DAOO, 'lion-dao/roar20/holders.json'));
  ...
  const hero = T(d.getElementById('hero'));
  ok('burn hero: supply ' + fmtBig(PH.token.total_supply) + ' pyROAR, ' + PH.holder_count + ' holders, largest at ' + PH.concentration.top1_pct.toFixed(2) + '%, gate Δ 0 (exact)', has(hero, fmtBig(PH.token.total_supply)) && has(hero, PH.holder_count.toLocaleString() + ' accounts hold a balance') && has(hero, LDN.fmt.pctPlain(PH.concentration.top1_pct, 2)) && has(hero, 'Σ balances = 0') && has(hero, '(exact)'), hero.slice(0, 400));
...
{ const { d, T, fetched } = await run('liondao/index.html', 'https://thealliancedao.com/liondao/', { lcd: (u) => (/publicnode|coingecko|dexscreener/.test(u) ? null : undefined), wait: 5000 });
  ...
  ok('page rev 2.4 in the footer', has(T(d.getElementById('page-rev')), 'Rev 2.6'));
  { const PH = J(path.join(DAOO, 'lion-dao/burn/holders.json')); const RH = J(path.join(DAOO, 'lion-dao/roar20/holders.json')); const bars = home.querySelectorAll('.ht-bar'); const py = bars[2].parentElement, r20 = bars[3].parentElement; const Tp = T(py), Tr = T(r20);
    ...
    ok('pyROAR note from the product: ' + PH.holder_count + ' accounts, largest at ' + PH.concentration.top1_pct.toFixed(1) + '%, top ten ' + PH.concentration.top10_pct.toFixed(1) + '% — chain facts only', has(Tp, PH.holder_count.toLocaleString() + ' accounts hold it') && has(Tp, 'at ' + LDN.fmt.pctPlain(PH.concentration.top1_pct, 1)) && has(Tp, 'hold ' + LDN.fmt.pctPlain(PH.concentration.top10_pct, 1)) && !/reported|theirs|224/.test(Tp), Tp.slice(-320));
