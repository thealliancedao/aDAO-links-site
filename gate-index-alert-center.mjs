...
console.log('=== index 4.32 alert center (lib 1.7.0 · alert vs activity) ===');
...
ok('S1 a sale + a listing + an epoch flip = activity only → NFTs and TLA tiles GREEN with grey counts (nothing needs a look)', Ra.tiles.find(t => t.key === 'nfts-adao').state === 'green' && Ra.tiles.find(t => t.key === 'nfts-adao').total === 2 && Ra.tiles.find(t => t.key === 'nfts-adao').alerts === 0 && Ra.tiles.find(t => t.key === 'tla').state === 'green' && Ra.tiles.find(t => t.key === 'tla').total === 1);
...
ok('S2 a chain-only listing and a threshold crossing ARE alerts → those tiles amber; the alert row sorts before the activity row', Rb.tiles.find(t => t.key === 'nfts-adao').state === 'amber' && Rb.tiles.find(t => t.key === 'nfts-adao').alerts === 1 && Rb.tiles.find(t => t.key === 'tla').state === 'amber' && Rb.tiles.find(t => t.key === 'nfts-adao').counters.find(c => c.key === 'market').rows[0].severity === 'alert');
