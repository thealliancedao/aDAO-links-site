...
const dom = new JSDOM(html, { url: 'https://thealliancedao.com/index.html', runScripts: 'dangerously', pretendToBeVisual: true, beforeParse(w) {
  ...
  w.eval(denomsSrc); w.eval(libSrc); w.eval(fs.readFileSync('lib/live-activity.js', 'utf8')); w.Denoms.load(stubFetch);   // 4.36: the feed lib too
  // 1.9.0 (owner): the NFTs aDAO tile is OFF on the home window unless opts.nfts. The rule gates below still exercise it, so the
  // gate turns it on for every build here and proves the default separately (N0).
  { const orig = w.AlertCenter.build; w.__acBuildOrig = orig; w.AlertCenter.build = (i, o) => orig(i, Object.assign({ nfts: true }, o || {})); }
  { const r0 = w.__acBuildOrig({ now: Date.now(), alertsDoc: { alerts: [] }, lpPools: [], props: { cards: [] }, items: [{ col: 'adao', kind: 'sale', token: 1, ts: Date.now() - 3600e3, label: 'Sold', sub: 'BBL' }] }, {});
    const r1 = w.__acBuildOrig({ now: Date.now(), alertsDoc: { alerts: [] }, lpPools: [], props: { cards: [] }, items: [{ col: 'adao', kind: 'sale', token: 1, ts: Date.now() - 3600e3, label: 'Sold', sub: 'BBL' }] }, { nfts: true });
    ok('N0 (1.9.0) without opts.nfts the build has no NFTs aDAO tile (Ecosystem · Props only); with it the tile is back', !r0.tiles.some(t => t.key === 'nfts-adao') && r1.tiles.some(t => t.key === 'nfts-adao'), r0.tiles.map(t => t.key)); }   // the page's include calls Denoms.load() before any feed builds
} });
const w = dom.window;
...
console.log('=== index 4.35 alert center (DEN map gone) ===');
...
const TAdoc = JSON.parse(fs.readFileSync(path.join(CORE, 'member-data/tla-alerts/current.json'), 'utf8'));   // 4.36: from TLA_CORE_DIR (was a hardcoded build path — the gate drift noted 2026-09-13)
...
ok('X2 (4.36) the four Boost listings the old feed re-announced on every API blip (#8149 · #1169 · #4291 · #1258) are NOT 2026-09 listing episodes — the chain dates them Dec 2025 / Apr 2026', (w.__activityEpisodes || []).length > 0 && !(w.__activityEpisodes || []).some(x => x.feed.slug === 'adao' && x.ep.kind === 'listing' && x.ep.venue === 'boost' && ['8149', '1169', '4291', '1258'].includes(String(x.ep.token)) && Date.parse(x.ep.ts) > Date.parse('2026-05-01')));
const sale = items26.find(x => x.kind === 'sale' && x.token == 745);
{ const s745 = (w.__activityItems || []).find(x => x.kind === 'sale' && String(x.token) === '745'); ok('X3 (4.36) the #745 sale legacy item (from its episode) carries seller + buyer + 200 bLUNA', s745 && s745.seller === 'terra1nj74mncupt0xhpxljls09gg7ufy0nx68fyarfy' && s745.buyer === 'terra1sw7x43lamdkm9gj0luzgzdym52y2sxcv7nk9hy' && s745.amt === 200 && s745.sym === 'bLUNA', s745 && [s745.seller, s745.buyer, s745.amt, s745.sym]); }
...
ok('X4 (4.36) the #745 sale episode carries both parties, 200 bLUNA, USD at the time and days on market (or "listed before the window")', (() => { const e = (w.__activityEpisodes || []).find(x => x.feed.slug === 'adao' && x.ep.kind === 'sale' && String(x.ep.token) === '745'); return e && e.ep.wallet === 'terra1nj74mncupt0xhpxljls09gg7ufy0nx68fyarfy' && e.ep.counterparty === 'terra1sw7x43lamdkm9gj0luzgzdym52y2sxcv7nk9hy' && e.ep.amount === 200 && e.ep.denom_symbol === 'bLUNA' && e.ep.usd > 0 && ('days_on_market' in e.ep); })());
ok('X5 (4.36) a priced single listing episode names its tier and carries the tier floor of its day (or null = no floor), never a floor of another class', (w.__activityEpisodes || []).filter(x => x.feed.kind === 'nft' && x.ep.kind === 'listing' && x.ep.count === 1 && x.ep.usd != null).every(x => x.ep.tier && ('floor_then' in x.ep)));
fwx.close(false);
...
ok('D2 (4.36) the feed builder assembles no stream by hand: no flows / transfers / tla-voting locks reads, no hand denom map — it reads <slug>/ledger/activity.json through lib/live-activity.js', /ledger\/activity\.json/.test(html) && !/grab\(`adao\/flows\//.test(html) && !/tla-voting\/events\/locks/.test(html) && !/const SYM = \{ uluna: 'LUNA'/.test(html));
...
const actRow = d.querySelector('[data-la-i]');
if (actRow) { actRow.dispatchEvent(new w.MouseEvent('click', { bubbles: true })); await new Promise(r => setTimeout(r, 50)); const det = d.querySelector(`[data-la-d="${actRow.dataset.laI}"]`);
  ok('L1 (4.36) clicking a Live Activity row renders its details card under it (who · transactions)', !det.classList.contains('hidden') && /Transactions/.test(det.textContent), det.textContent.slice(0, 120));
  actRow.dispatchEvent(new w.MouseEvent('click', { bubbles: true })); await new Promise(r => setTimeout(r, 50)); ok('L2 clicking again collapses it', det.classList.contains('hidden')); }
...
ok('L3 (4.36) every rendered row carries a tier class (featured · normal · quiet), a title and a tx link', [...d.querySelectorAll('#activity-list .la-row')].length > 0 && [...d.querySelectorAll('#activity-list .la-row')].every(r => /la-(featured|normal|quiet)/.test(r.className) && r.querySelector('.la-title') && r.querySelector('a[href*="chainsco.pe/terra2/tx/"]')));
// (4.29 inline-facts probes retired with the 4.36 episode feed)
// (4.29 inline-facts probes retired with the 4.36 episode feed)
ok('L4 (4.36) priced rows show "$x then · $y now ±%" on the right', (() => { const rows = [...d.querySelectorAll('#activity-list .la-row')]; const priced = rows.filter(r => r.querySelector('.la-usd')); return priced.length > 0 && priced.every(r => /\$[\d,.]+ then/.test(r.textContent)); })());
