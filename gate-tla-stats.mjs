...
check('N3 subnav has three tabs (overview · grades · pools); "tla" deep links land on pools', subnavItems && !subnavItems.some(t => t.id === 'tla') && subnavItems.some(t => t.id === 'pools' && t.label === 'Pools') && (() => { w.eval('switchToTab')('tla'); return d.getElementById('tab-pools').classList.contains('active'); })());
...
check('P7 each bucket has its liquidity & volume chart card back with the tier selector (All / Top / Mid / Low); the canvas is replaced by a loading note until the daily series arrives', d.querySelectorAll('#pools-by-bucket select.scale-select').length === 4 && (pb.textContent.match(/Daily liquidity/g) || []).length === 4 && /Top tier/.test(pb.textContent) && /Low tier/.test(pb.textContent));
