...
check('T4 Vote Advisor v2 renders: the earned rule stated, four bucket cards with NOW / RECOMMENDED bars, and "hold" today (no pool has 4 epochs yet)', ga && /votes are earned/.test(ga.textContent) && /4 consecutive epochs/.test(ga.textContent) && (ga.textContent.match(/NOW/g) || []).length === 4 && /hold/.test(ga.textContent) && /Earning/.test(ga.textContent) && !/copy proposal message/.test(ga.textContent), ga && ga.textContent.replace(/\s+/g, ' ').slice(0, 220));
w.toggleAdvisorPreview(); await new Promise(r => setTimeout(r, 100));
check('T5 preview at full streaks: at least one bucket recommends with reasons, the emissions line, and a proposal message on the gauge controller with bps that sum to 10000', (() => { const t = ga.textContent; const m = ga.querySelector('#advisor-msg'); if (!m) return false; const msgs = JSON.parse(m.textContent); return /recommend ·/.test(t) && /of LUNA emissions redirected/.test(t) && msgs.every(x => x.wasm.execute.contract_addr === 'terra1hfksrhchkmsj4qdq33wkksrslnfles6y2l77fmmzeep0xmq24l2smsd3lj' && x.wasm.execute.msg.vote.votes.reduce((s2, v) => s2 + v[1], 0) === 10000 && x.wasm.execute.msg.vote.votes.every(v => v[1] <= 5000)); })(), ga.textContent.replace(/\s+/g, ' ').match(/recommend ·[^N]{0,120}/)?.[0]);
w.toggleAdvisorPreview();
