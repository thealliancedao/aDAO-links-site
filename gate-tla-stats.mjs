...
check('M2 Vote Market grid: rows carry Votion\'s VP caption and the optimizer projection differs per pool', /Votion [\d,.KM]+/.test(vm.textContent) && (() => { const g = [...vm.textContent.matchAll(/\+([\d,.KM]+) VPshare/g)].map(m => m[1]); return g.length >= 3 && new Set(g).size >= 2; })(), [...vm.textContent.matchAll(/\+([\d,.KM]+) VPshare/g)].map(m => m[1]).slice(0, 5));
