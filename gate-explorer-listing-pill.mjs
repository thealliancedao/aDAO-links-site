...
{ const h = fs.readFileSync('nft-explorer-index.html', 'utf8'); const sv = (h.match(/nft-explorer-style\.css\?v=([\d.]+)/) || [])[1], av = (h.match(/nft-explorer-app\.js\?v=([\d.]+)/) || [])[1], rv = (h.match(/rev: '([\d.]+)'/) || [])[1];
  ok(`html: stylesheet + app cache-busted together (style ${sv} / app ${av}), footer rev ${rv} present`, sv && av && sv === av && !!rv, [sv, av, rv]); }   // 4.35: a relation, not a literal — the version bumps with every delivery
