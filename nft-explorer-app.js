...
function journeyRender(el, id, data) {
    ...
    const segHtml = (g) => { const acq = g.acquired; const how = acq ? (acq.how === 'mint' ? 'bought at mint for ' + money(acq.amount) : acq.how === 'free mint' ? 'minted free' : acq.how === 'sale' ? 'bought for ' + money(acq.amount) : 'arrived by transfer (basis unknown)') : (g.admin ? '' : 'basis unknown');
      ...
      const ended = g.ended_by === 'sale' ? 'sold' : g.ended_by === 'transfer' ? 'transferred away' : g.ended_by === 'admin' ? '' : (g.current ? '' : '');
      const stats = g.admin ? '' : ` · listed ${g.listings}×${g.price_changes ? ` · ${g.price_changes} price change${g.price_changes === 1 ? '' : 's'}` : ''}${g.days_on_market ? ` · ${g.days_on_market} days on market` : ''}`;
      const body = g.rows.map(r => r.kind === 'sale' ? `<li class="jr-sale-li">${saleCard(r)}</li>` : rowLi(r)).join('');
      return `<section class="jr-seg${g.admin ? ' jr-seg-admin' : ''}${g.current ? ' jr-seg-now' : ''}"><header class="jr-seg-head"><span class="jr-seg-n">${g.admin ? 'Treasury' : 'Holder ' + (S.segments.filter(x => !x.admin && x.index <= g.index).length)}</span> ${journeyWho(g.owner)}<span class="jr-seg-meta">${when}${how ? ' · ' + how : ''}${stats}${ended ? ' · ' + ended : ''}</span></header><ol>${body || '<li class="jr-minor"><span class="jr-h">no event on their watch yet</span></li>'}</ol></section>`; };
