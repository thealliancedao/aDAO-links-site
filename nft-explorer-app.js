...
//   with a sale card on each boundary: the seller's side (days on market, price changes, P&L USD + LUNA terms, days held) and
//   the buyer's (holdings now, paid then/now, LUNA-equivalent) — lib/nft-history 1.2.0 segments. LUNA terms read n/a when the
//   sale was paid in USDC/SOLID (no LUNA leg). Holdings are NOW; at-the-time is a by-wallet product still to build.
...
// --- Unified header picker (SPEC-unified-header): the VIEWING pill drives the
// Wallet tab. Selecting a wallet anywhere updates the wallet search; opening the
// Wallet tab with a selection loads it; clicking a leaderboard row selects in
// the header so every other page follows too.
window.addEventListener('tla:wallet', (e) => {
    const w = e && e.detail && e.detail.wallet;
    if (!walletSearchAddressInput) return;
    if (!w) return;                                   // cleared: leave the tab as-is
    walletSearchAddressInput.value = w;
    if (walletView && !walletView.classList.contains('hidden')) searchWallet();
});
...
function switchView(viewName, fromHistory = false) {
    ...
    if (viewName === 'wallet' && window.AddressPicker && walletSearchAddressInput) {
        const sel = AddressPicker.get();
        if (sel && sel.wallet && walletSearchAddressInput.value.trim() !== sel.wallet) {
            walletSearchAddressInput.value = sel.wallet;
            setTimeout(searchWallet, 0);   // after the view unhides
        }
    }
...
function journeyRender(el, id, data) {
    ...
    // 4.37: SECTIONS — one per holder (the segment the lib folded), a sale card on the boundary with the seller's and the
    // buyer's side of it. Holdings are the buyer's holdings NOW (nfts.json); "at the time of the buy" is a by-wallet product
    // still to be built, so it is not shown rather than guessed.
    const HOLD = (typeof buildHoldingsMap === 'function' && typeof allNfts !== 'undefined' && allNfts.some(n => n.owner)) ? buildHoldingsMap() : null;
    const holdingsText = (addr) => { if (!HOLD) return 'holdings now: not loaded yet'; const h = HOLD[addr]; if (!h || !h.held) return 'holds 0 aDAO NFTs now (exited)'; return `holds ${h.held} now · ${h.staked} staked · ${h.listed} listed · ${h.liquid} liquid`; };
    const rowLi = (r) => `<li class="jr-${journeyEsc(r.tone)}${r.gap ? ' jr-gap' : ''}"><span class="jr-day">${journeyEsc(r.day)}</span><span class="jr-h">${journeyEsc(r.headline)}</span><a class="jr-tx" href="https://chainsco.pe/terra2/tx/${journeyEsc(r.txhash)}" target="_blank" rel="noopener" title="${journeyEsc(r.txhash)}">tx ↗</a>${r.gap ? `<span class="jr-sub">${journeyEsc(r.gap_note)}</span>` : ''}${r.sub ? `<span class="jr-sub">${journeyEsc(r.sub)}</span>` : ''}</li>`;
    const money = (amt) => amt ? journeyEsc(amt.display) + (amt.usd_then != null ? ` (${journeyEsc(NH.fmtUsd(amt.usd_then))} then${amt.usd_now != null ? ` · ${journeyEsc(NH.fmtUsd(amt.usd_now))} now` : ''})` : '') : '?';
    const signed = (x, f) => x == null ? 'n/a' : (x >= 0 ? '+' : '−') + f(Math.abs(x));
    const saleCard = (r) => { const p = r.pnl || {}; const buyerAcq = r.buyer_segment != null && S.segments[r.buyer_segment] ? S.segments[r.buyer_segment].acquired : null;
      const sellerLine = p.basis_known
        ? `${r.episode ? `<b>${r.episode.days}</b> day${r.episode.days === 1 ? '' : 's'} on the market` : 'not listed on a venue'} · ${p.price_changes_note || ''}P&amp;L USD <b class="${p.usd_delta != null && p.usd_delta < 0 ? 'neg' : 'pos'}">${journeyEsc(signed(p.usd_delta, NH.fmtUsd))}${p.usd_pct != null ? ` (${p.usd_pct >= 0 ? '+' : '−'}${Math.abs(p.usd_pct).toFixed(0)}%)` : ''}</b> · LUNA terms <b class="${p.luna_delta != null && p.luna_delta < 0 ? 'neg' : 'pos'}">${p.luna_delta != null ? journeyEsc(signed(p.luna_delta, (x) => NH.fmtAmt(x) + ' LUNA')) + (p.luna_pct != null ? ` (${p.luna_pct >= 0 ? '+' : '−'}${Math.abs(p.luna_pct).toFixed(0)}%)` : '') : 'n/a'}</b>${p.received_luna_na ? ` <span class="jr-note">(${journeyEsc(p.received_luna_basis)})</span>` : ''} · held <b>${p.held_days}</b> days · paid ${money(p.paid)}${p.paid_how === 'free mint' ? ' free (mint)' : ''}`
        : `${r.episode ? `<b>${r.episode.days}</b> day${r.episode.days === 1 ? '' : 's'} on the market · ` : ''}cost basis unknown (arrived by transfer) — proceeds ${money(r.amount)}`;
      const buyerLine = `${journeyEsc(holdingsText(r.party ? r.party.addr : ''))} · paid ${money(r.amount)}${buyerAcq && buyerAcq.luna != null && r.amount && r.amount.symbol !== 'LUNA' ? ` ≈ ${journeyEsc(NH.fmtAmt(buyerAcq.luna))} LUNA at the time` : (buyerAcq && buyerAcq.luna_na ? ' · LUNA terms n/a (no LUNA leg)' : '')}`;
      return `<div class="jr-sale-card"><div class="jr-sale-head"><span class="jr-day">${journeyEsc(r.day)}</span><b>${journeyEsc(r.venue || 'Sale')} sale</b> · ${money(r.amount)} <a class="jr-tx" href="https://chainsco.pe/terra2/tx/${journeyEsc(r.txhash)}" target="_blank" rel="noopener">tx ↗</a></div>` +
        `<div class="jr-side"><span class="jr-role">Seller</span> ${journeyWho(r.from)}<div class="jr-side-body">${sellerLine}</div></div>` +
        `<div class="jr-side"><span class="jr-role">Buyer</span> ${journeyWho(r.party)}<div class="jr-side-body">${buyerLine}</div></div></div>`; };
    // 4.38: BOUNDARY TILES — the events that move the token between holders are tiles of their own, between the sections:
    //   MINTED (origin), SOLD (the big one: seller's side + buyer's side), HANDED TO (a plain transfer, slim). The holder
    //   sections underneath are just what each holder did with it — the tiles are what you scan for.
    const tileTx = (r) => `<a class="jr-tx" href="https://chainsco.pe/terra2/tx/${journeyEsc(r.txhash)}" target="_blank" rel="noopener" title="${journeyEsc(r.txhash)}">tx ↗</a>`;
    const tileHtml = (g) => { const r = g.boundary_row; if (!r) return '';
      if (r.boundary === 'sale') { const p = r.pnl || {}; const buyerAcq = g.acquired;
        const sellerLine = p.basis_known
          ? `${r.episode ? `<b>${r.episode.days}</b> day${r.episode.days === 1 ? '' : 's'} on the market · ` : ''}${S.segments[r.seller_segment] && S.segments[r.seller_segment].price_changes ? `${S.segments[r.seller_segment].price_changes} price change${S.segments[r.seller_segment].price_changes === 1 ? '' : 's'} · ` : ''}P&amp;L USD <b class="${p.usd_delta != null && p.usd_delta < 0 ? 'neg' : 'pos'}">${journeyEsc(signed(p.usd_delta, NH.fmtUsd))}${p.usd_pct != null ? ` (${p.usd_pct >= 0 ? '+' : '−'}${Math.abs(p.usd_pct).toFixed(0)}%)` : ''}</b> · LUNA terms <b class="${p.luna_delta != null && p.luna_delta < 0 ? 'neg' : 'pos'}">${p.luna_delta != null ? journeyEsc(signed(p.luna_delta, (x) => NH.fmtAmt(x) + ' LUNA')) + (p.luna_pct != null ? ` (${p.luna_pct >= 0 ? '+' : '−'}${Math.abs(p.luna_pct).toFixed(0)}%)` : '') : 'n/a'}</b>${p.received_luna_na ? ` <span class="jr-note">(${journeyEsc(p.received_luna_basis)})</span>` : ''} · held <b>${p.held_days}</b> days · paid ${money(p.paid)}${p.paid_how === 'free mint' ? ' free (mint)' : ''}`
          : `${r.episode ? `<b>${r.episode.days}</b> day${r.episode.days === 1 ? '' : 's'} on the market · ` : ''}cost basis unknown (arrived by transfer) — proceeds ${money(r.amount)}`;
        const buyerLine = `${journeyEsc(holdingsText(r.party ? r.party.addr : ''))} · paid ${money(r.amount)}${buyerAcq && buyerAcq.luna != null && r.amount && r.amount.symbol !== 'LUNA' ? ` ≈ ${journeyEsc(NH.fmtAmt(buyerAcq.luna))} LUNA at the time` : (buyerAcq && buyerAcq.luna_na ? ' · LUNA terms n/a (no LUNA leg)' : '')}`;
        return `<div class="jr-tile jr-tile-sale"><div class="jr-tile-art" aria-hidden="true"><span class="jr-tile-icon">💸</span><span class="jr-tile-ribbon">SOLD</span></div><div class="jr-tile-body"><div class="jr-tile-head"><span class="jr-day">${journeyEsc(r.day)}</span><b>${journeyEsc(r.venue || 'Sale')}</b> · <span class="jr-tile-amt">${money(r.amount)}</span> ${tileTx(r)}</div>` +
          `<div class="jr-side"><span class="jr-role">Seller</span> ${journeyWho(r.from)}<div class="jr-side-body">${sellerLine}</div></div>` +
          `<div class="jr-side"><span class="jr-role">Buyer</span> ${journeyWho(r.party)}<div class="jr-side-body">${buyerLine}</div></div></div></div>`; }
      if (r.boundary === 'mint') { const a = g.acquired; return `<div class="jr-tile jr-tile-mint"><div class="jr-tile-art" aria-hidden="true"><span class="jr-tile-icon">✨</span><span class="jr-tile-ribbon">MINTED</span></div><div class="jr-tile-body"><div class="jr-tile-head"><span class="jr-day">${journeyEsc(r.day)}</span><b>${a && a.how === 'mint' ? 'Bought at mint' : 'Minted free'}</b> · ${journeyWho(g.owner)}${a && a.amount ? ` · <span class="jr-tile-amt">${money(a.amount)}</span>` : ''}${a && a.luna != null && a.amount && a.amount.symbol !== 'LUNA' ? ` ≈ ${journeyEsc(NH.fmtAmt(a.luna))} LUNA` : ''} ${tileTx(r)}</div>${r.sub && !a.amount ? `<div class="jr-side-body">${journeyEsc(r.sub)}</div>` : ''}</div></div>`; }
      // handoff: a plain transfer between wallets
      return `<div class="jr-tile jr-tile-hand${r.gap ? ' jr-gap' : ''}"><div class="jr-tile-art" aria-hidden="true"><span class="jr-tile-icon">🤝</span></div><div class="jr-tile-body"><div class="jr-tile-head"><span class="jr-day">${journeyEsc(r.day)}</span><b>Handed to</b> ${journeyWho(g.owner)}${r.from ? ` <span class="jr-note">from ${journeyEsc(r.from.text)}</span>` : ''} ${tileTx(r)}</div>${r.gap ? `<div class="jr-side-body">⚠ ${journeyEsc(r.gap_note)}</div>` : ''}</div></div>`; };
    const segHtml = (g) => { const acq = g.acquired;
      const when = g.current ? `holding · <b>${g.held_days}</b> day${g.held_days === 1 ? '' : 's'} since ${journeyEsc(g.start_day)}` : `held <b>${g.held_days}</b> day${g.held_days === 1 ? '' : 's'} · ${journeyEsc(g.start_day)} → ${journeyEsc(g.end_day)}`;
      const ended = g.ended_by === 'sale' ? 'sold on' : g.ended_by === 'transfer' ? 'passed on' : '';
      const stats = g.admin ? '' : `${g.listings ? ` · listed ${g.listings}×` : ''}${g.price_changes ? ` · ${g.price_changes} price change${g.price_changes === 1 ? '' : 's'}` : ''}${g.days_on_market ? ` · ${g.days_on_market} days on market` : ''}`;
      const body = g.rows.map(rowLi).join('');
      const n = S.segments.filter(x => !x.admin && x.index <= g.index).length;
    ...
    const buildHolderRow = ({ address, ...stats }, rankCellHtml, isPinned) => {
        ...
        item.addEventListener('click', (e) => {
            ...
            if (address) {
                ...
                // Select in the unified header — its tla:wallet event drives the
                // wallet search (and every other page follows the same pick).
                if (window.AddressPicker) { AddressPicker.select(address); }
                else { walletSearchAddressInput.value = address; searchWallet(); }
            }
        });
        return item;
    };
...
// --- Snapshot Tool removed (Rev 4.21) ---
// ~700 lines of unreachable code: its trigger button no longer exists in the
// page. It also re-downloaded the 6.5MB nfts.json a second time and fetched a
// retired epoch file. History lives in git; the org products it duplicated are
