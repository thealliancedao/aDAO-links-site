#!/usr/bin/env node
// gate-test-3.mjs — the staged three-question page (test-3.html): every gate of tla-stats plus S1–S4. Usage: TLA_CORE_DIR=<tla-core> node gate-test-3.mjs
...
const html = fs.readFileSync(path.join(here, 'test-3.html'), 'utf8').replace(/<script[^>]*src=[^>]*><\/script>/g, '');
