...
ok('page: holder 2 section is the current one ("holding · 6 days") and quiet — no rows, says so', holders[1].classList.contains('jr-seg-now') && /holding · 6 days/.test(holders[1].textContent) && /held quietly/.test(holders[1].textContent) && !holders[1].querySelector('ol'), holders[1].textContent);
