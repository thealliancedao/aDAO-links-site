...
ok('html: modal has #modal-journey, loads /lib/nft-history.js, app + style cache-busted to 6.6, footer rev 4.37', html.includes('id="modal-journey"') && html.includes('/lib/nft-history.js?v=1.2.0') && html.includes('nft-explorer-app.js?v=6.6') && html.includes('nft-explorer-style.css?v=6.6') && html.includes("rev: '4.37'"));
...
{ const luna = JSON.parse(fs.readFileSync(path.join(CORE, 'price-history/series/LUNA.json'))).daily; const o2 = Object.assign({}, opts, { lunaUsdOn: (d) => luna[d] });
  ...
  ok('#745 holder 2 (current): acquired by sale for 200 bLUNA, holding 6 days, the sale row is the first item on their watch', H[1].current && H[1].acquired.how === 'sale' && H[1].held_days === 6 && H[1].rows[0].kind === 'sale' && H[1].rows[0].seller_segment === H[0].index && H[1].rows[0].buyer_segment === H[1].index, H[1]);
  ok('#745 seller\'s own rows never include the sale (it sits on the boundary, drawn once)', !H[0].rows.some(x => x.kind === 'sale'), H[0].rows.map(x => x.kind));
...
ok('page: 4 sections (2 treasury/admin, collapsed by default; 2 holders), holder 1 header says bought at mint · held 832 days · listed 1× · 694 days on market · sold', segs.length === 4 && holders.length === 2 && /Holder 1/.test(holders[0].textContent) && /held 832 days/.test(holders[0].textContent) && /bought at mint for 115 LUNA/.test(holders[0].textContent) && /listed 1×/.test(holders[0].textContent) && /694 days on market/.test(holders[0].textContent) && /sold/.test(holders[0].textContent), holders.map(h => h.querySelector('.jr-seg-head').textContent));
ok('page: holder 2 section is the current one: "holding · 6 days" · bought for 200 bLUNA, with the sale card as its first item', holders[1].classList.contains('jr-seg-now') && /holding · 6 days/.test(holders[1].textContent) && holders[1].querySelector('ol li:first-child .jr-sale-card'), holders[1].querySelector('.jr-seg-head') && holders[1].querySelector('.jr-seg-head').textContent);
const card = j && j.querySelector('.jr-sale-card');
ok('page: the sale card has a Seller side (694 days on the market · P&L USD −$57.2 (−77%) · LUNA terms +239 LUNA (+208%) · held 832 days · paid 115 LUNA) and a Buyer side (holdings now · paid 200 bLUNA · ≈ 354 LUNA at the time)', card && /BackBone Labs sale/.test(card.textContent) && /Seller/.test(card.textContent) && /694 days on the market/.test(card.textContent) && /P&L USD −\$57\.2 \(−77%\)/.test(card.textContent) && /LUNA terms \+239 LUNA \(\+208%\)/.test(card.textContent) && /held 832 days · paid 115 LUNA/.test(card.textContent) && /Buyer/.test(card.textContent) && /paid 200 bLUNA \(\$16\.76 then/.test(card.textContent) && /≈ 354 LUNA at the time/.test(card.textContent), card && card.textContent);
...
ok('page: 7 event rows in chain order across the sections, the sale drawn exactly once (as the card)', lis.length === 7 && j.querySelectorAll('.jr-sale-card').length === 1 && !lis.some(l => l.classList.contains('jr-sale') && !l.classList.contains('jr-sale-li')), lis.map(l => l.className));
