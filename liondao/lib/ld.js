...
(function (root, factory) {
  ...
  // ---------------------------------------------------------------- UI primitives
  var CSS = [
    ':root{--ld-accent:var(--t-accent,#ffe600);--ld-accent2:var(--t-accent2,#f59e0b);--ld-surface:var(--t-surface,#161616);--ld-surface2:var(--t-surface2,#101010);--ld-border:var(--t-border,#4a4000);--ld-text:var(--t-text,#e5e7eb);--ld-muted:#a3a3a3;--ld-dim:#737373;--ld-font:var(--t-font,"Press Start 2P",Inter,sans-serif);--ld-good:#7ee787;--ld-bad:#ff7b72}',
    ...
    '.ld-h{font-family:var(--ld-font);font-size:.72rem;color:var(--ld-accent);letter-spacing:.02em;margin:0 0 .75rem;display:flex;align-items:baseline;gap:.75rem;flex-wrap:wrap;line-height:1.8}.ld-h small{font-family:Inter,sans-serif;font-size:.78rem;color:var(--ld-muted);letter-spacing:0}',
    '.ld-sec{margin:0 0 1.5rem}',
    '.ld-tile{background:var(--ld-surface);border:2px solid var(--ld-border);padding:.9rem 1rem;min-width:0}.ld-tile.ld-hard{box-shadow:6px 6px 0 var(--ld-accent)}.ld-tile.ld-link{cursor:pointer;text-decoration:none;display:block}.ld-tile.ld-link:hover,.ld-tile.ld-link:focus-visible{border-color:var(--ld-accent)}',
    '.ld-k{font-size:.78rem;color:var(--ld-muted);margin:0 0 .35rem}.ld-v{font-family:var(--ld-font);font-size:1.05rem;color:#fff;line-height:1.5;word-break:break-word}.ld-v.ld-v-sm{font-size:.8rem}.ld-v.ld-v-lg{font-size:1.6rem}.ld-s{font-size:.78rem;color:var(--ld-muted);margin-top:.35rem;line-height:1.45}.ld-src{font-size:.7rem;color:var(--ld-dim);margin-top:.5rem}',
    '.ld-unk{display:inline-block;font-family:var(--ld-font);font-size:.6rem;color:var(--ld-accent);border:1px dashed var(--ld-accent);padding:.35rem .5rem;line-height:1;cursor:help;vertical-align:middle}',
    ...
    '.ld-wal{display:inline-flex;align-items:center;gap:.4rem;font-size:.75rem;color:var(--ld-text);border:1px solid var(--ld-border);padding:.15rem .5rem;white-space:nowrap;text-decoration:none}.ld-wal b{color:var(--ld-accent);font-weight:600}.ld-wal:hover{border-color:var(--ld-accent)}',
    ...
    '.ld-note{font-size:.8rem;color:var(--ld-muted);padding:.75rem;border:1px dashed var(--ld-border);line-height:1.5}.ld-good{color:var(--ld-good)}.ld-bad{color:var(--ld-bad)}.ld-acc{color:var(--ld-accent)}',
  ...
  var ui = {
    ...
    stat: function (o) { var inner = '<div class="ld-k">' + esc(o.label) + '</div><div class="ld-v' + (o.size === 'lg' ? ' ld-v-lg' : (o.value != null && String(o.value).length > 12 ? ' ld-v-sm' : '')) + '">' + (o.value == null ? ui.unk(o.reason) : esc(o.value)) + '</div>' + (o.sub ? '<div class="ld-s">' + o.sub + '</div>' : '') + (o.source ? '<div class="ld-src">' + esc(o.source) + '</div>' : ''); return o.href ? '<a class="ld-tile ld-link' + (o.hard ? ' ld-hard' : '') + '" href="' + esc(o.href) + '">' + inner + '</a>' : '<div class="ld-tile' + (o.hard ? ' ld-hard' : '') + '">' + inner + '</div>'; },
    ...
    wallet: function (w, opts) { opts = opts || {}; var a = w.address || w; var label = w.label || fmt.short(a); var role = w.role || ''; return '<a class="ld-wal ld-role-' + esc(role) + '" href="' + esc(opts.href || ('https://chainsco.pe/terra2/address/' + a)) + '" target="_blank" rel="noopener" title="' + esc(a) + '"><span class="ld-dot"></span><b>' + esc(label) + '</b>' + (opts.role !== false && role ? '<span style="color:var(--ld-dim)">' + esc(role) + '</span>' : '') + '</a>'; },
  ...
  return { VERSION: VERSION, ready: ready, mount: mount, positions: positions, nap: nap, collection: collection, lcd: { get: lcdGet, paged: lcdPaged, smart: smart }, jsonFetch: jsonFetch,
