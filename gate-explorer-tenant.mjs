...
async function bootExplorer(appSrc, tenant, withCtx, opts_analytics, nftcDir) {
  ...
  const snap = { traitFilters: inner('trait-filters-container'), status: inner('status-filters-grid'), toggles: inner('trait-toggles-container'), walletToggles: inner('wallet-trait-toggles-container'),
    ...
    ranked: w.__g.ranked(), owners: w.__g.owners(), names: w.__g.names(), namedTop: w.__g.namedTop(), sysLabels: w.__g.sysLabels(), hero: (q('collection-hero') || { textContent: '' }).textContent.replace(/\s+/g, ' ').trim() || null };
  let analytics = null;
...
ok('liondao: the collection hero renders above the tabs — mark, "pixeLions", "Lion DAO", the registry tagline, Supply 5,000 · Holders · Listed · Floor', P.hero && /pixeLions/.test(P.hero) && /Lion DAO/.test(P.hero) && /pixel lions/i.test(P.hero) && /Supply\s*5,000/.test(P.hero) && /Holders/.test(P.hero) && /Listed/.test(P.hero) && /Floor/.test(P.hero), P.hero && P.hero.slice(0, 200));
...
{ const h = await header('liondao', true); const st = h.w.document.getElementById('sh-tenant-theme').textContent;
  ok('header (liondao): the page colour outranks the lib\'s own html,body !important rule, the mesh backdrop is repainted in the tenant colours, the aDAO Lore tab and the New-here pills are gone (tenants.json nav)', /body\.bg-gray-900\{background-color:#ffe600!important/.test(st) && /\.bg-mesh\{background:radial-gradient/.test(st) && !h.el.querySelector('.sh-tab[data-page="adao-lore"]') && !h.el.querySelector('.sh-nh-wrap') && !!h.el.querySelector('.sh-tab[data-page="dao"]'), [!!h.el.querySelector('.sh-tab[data-page="adao-lore"]'), !!h.el.querySelector('.sh-nh-wrap')]); }
