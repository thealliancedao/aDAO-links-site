...
async function bootExplorer(appSrc, tenant, withCtx, opts_analytics, nftcDir) {
  ...
  const snap = { traitFilters: inner('trait-filters-container'), status: inner('status-filters-grid'), toggles: inner('trait-toggles-container'), walletToggles: inner('wallet-trait-toggles-container'),
    ...
    firstImg: (d.querySelector('#nft-gallery .nft-card img') || { getAttribute: () => null }).getAttribute('src'), firstFallback: (d.querySelector('#nft-gallery .nft-card img') || { getAttribute: () => null }).getAttribute('data-fallback'),
...
ok('liondao: the first card\'s image comes from the manifest\'s IPFS pattern, with a second-gateway fallback (dweb.link) wired on error', P.firstImg && /^https:\/\/ipfs\.io\/ipfs\/[a-z0-9]+\/\d+\.png$/.test(P.firstImg) && /\.ipfs\.dweb\.link\//.test(P.firstFallback || ''), [P.firstImg, P.firstFallback]);
