...
console.log('=== slippage Rev 3.0 — trade planner on committed reserves ===');
{
  ...
  check('S3 impact bound 3.00% (≤, curved) and cost = impact ∘ 2×0.3% fees ≈ 3.58%', r[0] && Math.abs(r[0].impactPct - 3.00) < 0.05 && r[0].curvedAny && Math.abs(r[0].costPct - (1 - (1 - r[0].impactPct / 100) * 0.997 * 0.997) * 100) < 1e-9, r[0] && [r[0].impactPct, r[0].costPct]);
  ...
  check('S9 ladder carries the rungs + current size marker + tranche column; $1,000 row reads ~17.4% bound', /\$20/.test(ladder) && /\$5,000/.test(ladder) && /◂/.test(ladder) && (() => { const row = [...d.querySelectorAll('#ladder tr')].find(tr => /^\$1,000/.test(tr.textContent.trim())); return row && /≤17\./.test(row.textContent); })(), ladder.slice(0, 200));
  ...
  check('S11 SOLID context: peg line shows the LUNA-SOLID pool under $1', (() => { T.store.to = 'SOLID'; T.renderPlanner(); const t = d.getElementById('plan-ctx').textContent; return /peg:/.test(t) && /LUNA-SOLID Astro \$0\.99/.test(t) && /trading under \$1/.test(t); })(), d.getElementById('plan-ctx').textContent.slice(-300));
