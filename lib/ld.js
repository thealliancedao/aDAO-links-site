...
(function (root, factory) {
  ...
  var ui = {
    ...
    addr: function (a, opts) { opts = opts || {}; if (!a) return ''; if (!opts.label) { var nm = nameOf(a); if (nm) { opts.label = nm.label; opts.kind = nm.kind; } }   /* 1.2.0: one name source — roster, then the address catalog (entities + DAODAO handles) */ var sol = opts.chain === 'solana' || /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(a) && a.indexOf('terra') !== 0; var ex = sol ? 'https://solscan.io/account/' + encodeURIComponent(a) : 'https://chainsco.pe/terra2/address/' + a; var search = sol ? null : '/address-catalog.html?address=' + encodeURIComponent(a);
      return '<span class="ld-addr" title="' + esc(a) + '">' + (opts.label ? '<b class="ld-nm ld-nm-' + esc(opts.kind || 'x') + '" title="' + (opts.kind === 'handle' ? 'DAODAO profile handle' : opts.kind === 'entity' ? 'registry entity' : opts.kind === 'roster' ? 'tenant roster' : 'name') + '">' + esc(opts.label) + '</b> ' : '') + '<code>' + esc(opts.full ? a : fmt.short(a)) + '</code><button type="button" class="ld-copy" data-copy="' + esc(a) + '" title="copy the full address" aria-label="copy"><i class="fa-regular fa-copy"></i></button>' + (search ? '<a class="ld-go" href="' + esc(search) + '" title="search this address on the site"><i class="fa-solid fa-magnifying-glass"></i></a>' : '') + '<a class="ld-go" href="' + esc(ex) + '" target="_blank" rel="noopener" title="' + (sol ? 'Solscan' : 'Chainscope') + '"><i class="fa-solid fa-arrow-up-right-from-square"></i></a></span>'; },
  ...
  function mount(o) {
    ...
    return ready().then(function (r) { return loadNames().then(function () { return r; }); });   /* 1.2.0: names before first paint */
  }
  var NAMES = { map: null, loading: null };
  function loadNames() { if (NAMES.loading) return NAMES.loading; NAMES.loading = jsonFetch(CORE + 'catalog/snapshots/current.json', { timeoutMs: 30000 }).then(function (c) { var m = {}; var t = M.tenant || {}; Object.keys(t.wallets || {}).forEach(function (a) { m[a] = { label: t.wallets[a].label, kind: 'roster' }; }); if (t.validator && t.validator.account) m[t.validator.account] = { label: 'validator account', kind: 'roster' };
  ...
  return { VERSION: VERSION, ready: ready, mount: mount, _copyFallback: copyFallback, nameOf: nameOf, loadNames: loadNames, positions: positions, nap: nap, collection: collection, lcd: { get: lcdGet, paged: lcdPaged, smart: smart }, jsonFetch: jsonFetch,
