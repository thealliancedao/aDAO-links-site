...
// 2026-09-10 (owner): SIGNAL proposals — zero executable messages — are safe BY DEFINITION (nothing executes on pass).
// isSignal(card) is the one test; signalHtml(card) replaces the ledger + decoded + precedent panels with a single green
// line, and signalChip() is the overview badge. The moment a proposal carries a message it is not a signal and gets the
// full audit — the test is the message count, never the title.
(function () {
    'use strict';
    ...
    const isSignal = (card) => !!card && !(Array.isArray(card.rawMsgs) && card.rawMsgs.length);
    const signalChip = () => '<span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300" title="Text-only proposal: no executable messages, nothing runs on pass"><i class="fas fa-comment-dots mr-1"></i>SIGNAL</span>';
    const signalHtml = (card) => '<div class="mt-2 rounded border border-emerald-500/30 bg-emerald-500/[0.06] px-2 py-1.5 text-[11px]"><div class="font-bold text-[10px] uppercase tracking-wider mb-0.5 text-emerald-300"><i class="fas fa-comment-dots mr-1"></i>Signal · text only · ' + signalChip() + '</div>' +
        '<div class="text-gray-300">This proposal carries <b>no executable messages</b>. Passing it changes nothing on chain — no funds move, no contract is called — so there is nothing to audit. It is a recorded statement of intent; any follow-up that moves funds must come as its own proposal and will get the full audit here.</div>' +
        '<div class="mt-1 text-[9px] text-gray-500">Test is the message count on chain, not the title. Read the text and vote on the intent.</div></div>';
    const ledgerHtml = (lg, card) => {
        if (isSignal(card)) return signalHtml(card);
    ...
    window.PropAudit = {
        ...
        isSignal, signalChip, signalHtml,
    };
})();
