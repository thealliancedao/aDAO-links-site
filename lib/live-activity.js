...
(function (root) {
  'use strict';
  const VERSION = '1.1.0';   // 1.1.0 (owner, first live look): lock top-ups and dust locks are housekeeping (off unless big); a lock add is 'big' by what was ADDED (USD), not by the lock it landed in; backing_add rows out of the feed; rows in an off chip still show when featured; lock ids are plain text
  const DEFAULTS = {   // mirrored in alert-thresholds.json `live_activity`; the file wins when present
    ...
    lock_new_big_vp: 100000, lock_unlock_big_vp: 100000, lock_add_big_usd: 1000, lock_new_min_vp: 1000, mass_count: 10, big_sale_usd: 500, fold_ids_shown: 5,
  };
  const KIND_GROUPS = [   // toggle chips, in this order; each maps to episode kinds
    ...
    ['break', 'Breaks', ['break']], ['transfer', 'Transfers', ['transfer', 'lock_transfer', 'mint']],
    ['locks', 'Locks', ['lock_new', 'lock_unlock']], ['housekeeping', 'Lock housekeeping', ['lock_restructure', 'lock_add']],   // 1.1.0: top-ups are housekeeping; a big one is featured and shows regardless of the chip; backing_add (the DAO's routine backing deposits) has no chip and stays out of the feed
  ];
  ...
  function classify(ep, feed, ctx) {
    ...
    else if (k === 'lock_add') { if ((ep.usd || 0) >= cfg.lock_add_big_usd) { tier = 'featured'; why = `${usd(ep.usd)} added to a lock`; } else { tier = 'quiet'; why = 'a top-up (what was added is small — the lock it landed in may be big)'; } }
    else if (k === 'lock_new') { if (f.has('new_voter')) { tier = 'featured'; why = 'a wallet new to TLA voting'; } else if ((ep.vp || 0) >= cfg.lock_new_big_vp) { tier = 'featured'; why = `new lock of ${N(ep.vp, 0)} VP`; } else if ((ep.vp || 0) < cfg.lock_new_min_vp) { tier = 'quiet'; why = `dust lock${ep.count > 1 ? 's' : ''} (${N(ep.vp, 0)} VP total) — housekeeping`; } }
  ...
  function rowHtml(ep, feed, ctx, i) {
    ...
    const idLink = (id) => feed.explorer && ctx.tokenUrl ? `<a href="${esc(ctx.tokenUrl(feed, id))}" class="la-id" onclick="event.stopPropagation()">#${esc(id)}</a>` : `<span class="la-id">#${esc(id)}</span>`;   // 1.1.0: a lock id links nowhere (no lock explorer yet) — never to a chainscope 'address'
    const idsHtml = L.ids.length ? `<span class="la-ids">${L.ids.slice(0, L.shown).map(idLink).join(' ')}${L.ids.length > L.shown ? ` <button type="button" class="la-more" data-la-more="${i}" onclick="event.stopPropagation(); const r = this.closest('[data-la-i]'); const d = r && r.nextElementSibling; if (d) { d.classList.toggle('hidden'); }">+${L.ids.length - L.shown} more</button>` : ''}</span>` : '';
    const right = [];
  ...
  function detailsHtml(ep, feed, ctx) {
    ...
    if (ep.count > 1) rows.push([`${ep.count} tokens`, L.ids.map(id => feed.explorer && ctx.tokenUrl ? `<a href="${esc(ctx.tokenUrl(feed, id))}" class="la-id">#${esc(id)}</a>` : `<span class="la-id">#${esc(id)}</span>`).join(' ')]);
    ...
    if (ep.count === 1 && ep.token && ctx.tokenUrl && feed.explorer) rows.push(['Journey', `<a href="${esc(ctx.tokenUrl(feed, ep.token))}" class="text-cyan-300 hover:underline">open #${esc(ep.token)} in the explorer ↗</a>`]);
