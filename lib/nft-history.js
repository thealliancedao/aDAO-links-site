/* lib/nft-history.js 1.1.1 (2026-09-18) — 1.1.1: operator moves (an admin wallet moving a token it never held) read as such, no ⚠; distribution-wallet labels from the manifest · 1.1.0: P&L per round trip (USD + LUNA terms) and the current holder's basis; treasury transfers are admin rows
...
(function (root, factory) {
  ...
  function fold(records, opts) {
    ...
    const acquire = (r, amt, how) => {   // a new owner's cost basis, from the record that made them the owner
      ...
      acq = { ts: r.ts, day: dayOf(r.ts), amount: paid, usd: paid ? paid.usd_then : (how === 'free mint' ? 0 : null), luna: le ? le.luna : null, luna_basis: le ? le.basis : null, how, known: how !== 'transfer' };
    };
    const realize = (r, amt) => {   // this owner sold: paid → received, two ways
      ...
      p.text = pnlText(p); s.realized.push(p); return p;
    };
    for (let i = 0; i < recs.length; i++) {
      ...
      switch (r.kind) {
        ...
        case 'transfer': {
          ...
          if (isAdmin(r.to) || isAdmin(r.from)) { push(r, { tone: 'admin', headline: 'Transferred to ' + ((to && to.label) || (to ? to.text : '?')), sub: from ? 'from ' + from.text : null, party: to, from }); seen(r); acq = null; break; }   // stock going back / out of the treasury: an admin row, not a hand change
        ...
        case 'sale': {
          ...
          push(r, { tone: 'sale', headline: 'Sold to ' + (buyer ? buyer.text : '?') + ' for ' + (amt ? amt.display : '?') + (e ? ' · ' + e.days + ' day' + (e.days === 1 ? '' : 's') + ' on the market' : ''), sub: usdLine(amt) + (seller ? ' · by ' + seller.text : ''), pnl: p, amount: amt, party: buyer, from: seller, episode: e, venue: venueName(r, o) });
          s.sales++; s.last_sale = { ts: r.ts, amount: amt, to: buyer, from: seller, days_on_market: e ? e.days : null, pnl: p }; setOwner(r.to, r, 'sale'); seen(r); acquire(r, amt, 'sale');
          break; }
  ...
  function pnlText(p) {   // the sale row's second line, two ways
    ...
    const luna = p.luna_delta != null ? 'LUNA terms ' + fmtSigned(p.luna_delta, (x) => fmtAmt(x) + ' LUNA') + (p.luna_pct != null ? ' (' + fmtPct(p.luna_pct) + ')' : '') : (p.paid_luna === 0 && p.received_luna != null ? 'LUNA terms +' + fmtAmt(p.received_luna) + ' LUNA (free mint)' : 'LUNA terms n/a');
  ...
  return { fold, shardOf, optsFromManifest, amountOf, lunaEq, fmtUsd, fmtAmt, RELIST_H, version: '1.1.1' };
});
