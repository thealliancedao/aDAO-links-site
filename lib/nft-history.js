/* lib/nft-history.js 1.2.0 (2026-09-18) — 1.2.0: SEGMENTS (one per holder: how it began, what happened on their watch, how it ended, their P&L); LUNA terms only when a LUNA-family token changed hands (USDC/SOLID → n/a) · 1.1.1: operator moves (an admin wallet moving a token it never held) read as such, no ⚠; distribution-wallet labels from the manifest · 1.1.0: P&L per round trip (USD + LUNA terms) and the current holder's basis; treasury transfers are admin rows
...
(function (root, factory) {
  ...
  function fold(records, opts) {
    ...
    const push = (r, row) => { const x = Object.assign({ ts: r.ts, day: dayOf(r.ts), height: r.height, txhash: r.txhash, kind: r.kind, segment: seg ? seg.index : null }, row); rows.push(x); if (seg && x.kind !== 'sale') seg.rows.push(x); return x; };
    ...
    const setOwner = (addr, r, why) => { if (!addr || isAdmin(addr)) return; if (owner && owner !== addr) s.hands_changed++; if (!s.first_owner) s.first_owner = addr; if (!seg || (seg.owner && seg.owner.addr !== addr) || seg.admin) { if (seg) closeSeg(r, why === 'sale' ? 'sale' : 'transfer'); openSeg(r, addr, why); } owner = addr; };
    ...
    for (let i = 0; i < recs.length; i++) {
      ...
      switch (r.kind) {
        ...
        case 'sale': {
          ...
          s.sales++; s.last_sale = { ts: r.ts, amount: amt, to: buyer, from: seller, days_on_market: e ? e.days : null, pnl: p }; setOwner(r.to, r, 'sale'); seen(r); acquire(r, amt, 'sale'); rows[rows.length - 1].buyer_segment = seg ? seg.index : null; if (seg) seg.rows.unshift(rows[rows.length - 1]);
          break; }
  ...
  return { fold, shardOf, optsFromManifest, amountOf, lunaEq, fmtUsd, fmtAmt, RELIST_H, version: '1.2.0', LUNA_FAMILY };
});
