...
(function (root, factory) {
  ...
  function renderSupply(sec, D) {
    var tiles = (sec.bars || []).map(function (b) {
      ...
      return '<div class="ht-tile"><div class="ht-k">' + esc(b.label) + (b.total_key ? ' · of <b>' + (total == null ? unk(b.total_reason) : esc((b.total_fmt ? fmt[b.total_fmt] : fmt.big)(total))) + '</b>' : '') + '</div>' + bar(parts, total, D, { hasTotal: !!b.total_key }) + (b.note ? '<div class="ht-src">' + fill(b.note, D) + '</div>' : '') + '</div>';
    }).join('');
