...
(function (root, factory) {
  ...
  'use strict';
  var VERSION = '1.9.0';   // 1.9.0 (owner 2026-09-26: "make sure trends are enabled so users can pull them up"): a chart button on every tile whose number has a kept series — prices (network-and-prices series, the daily market series, ROAR20's hourly capture), supplies and stakes (history/daily), the DAO's value by section (positions/daily), floors (each collection's floor-history), holder counts. It opens a chart with 7d / 30d / 90d / All, hover for each point, the change over the range and the source; a series still filling says how many days it has. 1.8.0 (owner 2026-09-26: Burning Lions into the marketplace sections): its listings join All Current Listings under their own class (inventory nfts.json, real owner per token), the venue cards show a Burning Lions floor row, a listing opens the right collection in the explorer; its holders count real owners (a lion listed on BBL belongs to its seller, not to BBL). 1.7.0 (owner 2026-09-26: "add market cap, TVL, how much money kills a zero; for NFTs market cap, the mark, last sale vs floor — keep the tiles uniform"): every market card carries the same three fact rows under its price. Tokens: market cap · TVL (its DEX pools) · kill a zero (the buy that takes the price 10×, constant-product). NFTs: market cap (floor × minted) · mark (midpoint of floor and last sale) · last sale (USD at the sale, when, vs today's floor). A row with no source says why on hover. 1.6.3 (owner 2026-09-26: "knock them all out"): Burning Lions' market from its own inventory snapshot (nft-inventory D.3 → nft-collections/burning-lions/snapshots/floor-history.json): the card shows the floor and its 24h move once a row exists, the minted count until then (value_fallback_key · what_key); pyROAR's 24h from the daily market history (history/daily.json 1.2 rows). 1.6.2 (owner 2026-09-26: "still don't see anything for ROAR20 or Burning Lions in the header"): the Burning Lions card shows what the chain answers today — minted of stated, holders (owner_of per token) — and says the floor waits on a marketplace read; a card can name its middle slot (chg_label). 1.6.1: ROAR20's market from the hourly capture (ally-positions 1.3.0 → <dao>/roar20/market.json) — the browser's DexScreener read is now only the override; pyROAR's logo and the burn wording follow the festival's published mechanics.
  ...
  function loadSources(cfg, ctx, onSource) {
    ...
    if (dao) { J('pyroar_holders', jsonFetch(DAOO + dao + '/burn/holders.json')); J('roar20_holders', jsonFetch(DAOO + dao + '/roar20/holders.json')); J('roar20_market', jsonFetch(DAOO + dao + '/roar20/market.json')); J('hist_markets', jsonFetch(DAOO + dao + '/history/markets.json')); }   /* 1.6.3: the daily market series (ally-positions 1.4.0 history-forward) */   // 1.6.1: the hourly market capture
    if (prim) {
    ...
    if (T.burning_lions && T.burning_lions.contract) { J('lcd_bl_num_tokens', smart(lcd, T.burning_lions.contract, { num_tokens: {} })); J('lcd_bl_info', smart(lcd, T.burning_lions.contract, { contract_info: {} }));
      ...
      J('bl_floor', jsonFetch(NFTC + 'burning-lions/snapshots/floor-history.json'));
      J('bl_nfts', jsonFetch(NFTC + 'burning-lions/snapshots/nfts.json'));   /* 1.8.0: 7 records — listings + real owners */   /* 1.6.3: the collection's own inventory (a 404 until its first run) */
  ...
  function derive(S, cfg, ctx) {
    ...
    // 1.6.3: the daily market series — pyROAR's move against yesterday's row (the pair is read live; the series keeps one row a day)
    var HM = S.hist_markets && S.hist_markets.days ? S.hist_markets.days : null; var hmY = HM ? HM[new Date(Date.now() - 864e5).toISOString().slice(0, 10)] : null;
    D.history = { markets_days: HM ? Object.keys(HM).length : 0, markets_first: S.hist_markets ? S.hist_markets.first_day : null };
    if (hmY && isNum(hmY.pyroar_usd) && isNum(D.pyroar.price_usd) && hmY.pyroar_usd > 0) { D.pyroar.chg_24h_pct = (D.pyroar.price_usd / hmY.pyroar_usd - 1) * 100; D.pyroar.market_reason = null; }
    else if (HM) D.pyroar.market_reason = 'the daily market series started ' + (S.hist_markets.first_day || 'today') + ' — the 24h move needs yesterday\'s row';
    ...
    // 1.6.3: the inventory's daily floor rows — the lowest listing across tiers (one-of-ones have no tiers that matter)
    var blRows = S.bl_floor && Array.isArray(S.bl_floor.rows) ? S.bl_floor.rows : []; var blF = function (r) { if (!r || !r.per_tier) return null; var v = Object.keys(r.per_tier).map(function (k) { return r.per_tier[k] && r.per_tier[k].listing_floor_usd; }).filter(isNum); return v.length ? Math.min.apply(null, v) : null; };
    var blListed = function (r) { if (!r || !r.per_tier) return null; return Object.keys(r.per_tier).reduce(function (t, k) { return t + ((r.per_tier[k] && r.per_tier[k].listed_count) || 0); }, 0); };
    var b1 = blRows.length ? blRows[blRows.length - 1] : null, b0 = blRows.length > 1 ? blRows[blRows.length - 2] : null;
    D.bl.floor_usd = blF(b1); D.bl.listed = b1 ? blListed(b1) : null; D.bl.floor_date = b1 ? b1.date : null;
    D.bl.floor_chg_24h_pct = isNum(blF(b1)) && isNum(blF(b0)) && blF(b0) > 0 ? (blF(b1) / blF(b0) - 1) * 100 : null;
    if (b1) D.bl.market_reason = D.bl.floor_usd == null ? 'none listed on ' + b1.date + ' (the collection inventory)' : (b0 ? null : 'one floor row so far — the 24h move needs two days');
    D.bl.what = D.bl.floor_usd != null ? 'floor' : 'minted';
    ...
    // 1.8.0: the inventory's records — a listed lion's holder is its SELLER (real_owner), not the marketplace contract owner_of names
    var BN = S.bl_nfts && Array.isArray(S.bl_nfts.records) ? S.bl_nfts.records : null;
    if (BN && D.bl.minted == null && BN.length) { D.bl.minted = BN.length; D.bl.supply_text = fmt.int(BN.length) + (isNum(D.bl.planned) ? ' of ' + fmt.int(D.bl.planned) : ''); D.bl.gap_note = isNum(D.bl.planned) && BN.length < D.bl.planned ? fmt.int(D.bl.planned - BN.length) + ' not on this contract' : ''; D.bl.supply_source = 'the collection inventory (the cw721 did not answer live)'; }   /* 1.9.0 */
    if (BN) { var ro = {}; BN.forEach(function (r) { var o = r.real_owner || r.owner; if (o) ro[o] = 1; }); D.bl.holders = Object.keys(ro).length; D.bl.holders_reason = null;
      D.bl.listing_rows = BN.filter(function (r) { return r.listing && isNum(r.listing.price_usd); }).map(function (r) { return { id: String(r.id), usd: r.listing.price_usd, rank: null, col: 'burning-lions', venue: r.bbl_listed ? 'bbl' : r.atrium_listed ? 'atrium' : r.boost_listed ? 'boost' : String(r.listing.marketplace || '').toLowerCase() || null, chain_only: false }; }); }
    // 1.7.0: the three fact rows under each market card — same rows for every token, same rows for every NFT
    //   kill a zero = the buy that takes the price 10× in constant-product (x·y=k) pools: the quote side must grow √10×, so the
    //   cost is (√10 − 1) × the quote side = 1.081 × TVL (TVL = both sides). Ignores fees, sellers into the move, and pools that are
    //   not x·y=k (stable / concentrated); several pools of one token are treated as one (arbitrage keeps them together).
    var KZ = Math.sqrt(10) - 1;
    var kz = function (tvl) { return isNum(tvl) && tvl > 0 ? KZ * tvl / 2 : null; };
    D.roar.tvl_usd = isNum(D.roar.in_pools) && isNum(D.roar.price_usd) ? 2 * D.roar.in_pools * D.roar.price_usd : null;
    D.roar.tvl_reason = !isNum(D.roar.in_pools) ? 'the pools\' ROAR balances were not read' : 'no ROAR price';
    D.roar.mcap_usd = D.roar.fdv_ours; D.roar.kill_zero_usd = kz(D.roar.tvl_usd);
    D.roar.tvl_note = isNum(D.roar.tvl_usd) ? 'ROAR in ' + (D.roar.pools_read || 0) + ' registered pool' + (D.roar.pools_read === 1 ? '' : 's') + ' × 2 (x·y=k: both sides are worth the same)' : null;
    D.pyroar.mcap_usd = isNum(D.pyroar.price_usd) && isNum(D.pyroar.total_supply) ? D.pyroar.price_usd * D.pyroar.total_supply : null;
    D.pyroar.tvl_usd = isNum(D.pyroar.pair_depth_usd) ? D.pyroar.pair_depth_usd : null; D.pyroar.kill_zero_usd = kz(D.pyroar.tvl_usd);
    if (D.roar20) { D.roar20.mcap_usd = isNum(D.roar20.market_cap_usd) ? D.roar20.market_cap_usd : (isNum(D.roar20.price_usd) && isNum(D.roar20.supply) ? D.roar20.price_usd * D.roar20.supply : null);
      D.roar20.tvl_usd = isNum(D.roar20.liquidity_usd) ? D.roar20.liquidity_usd : null; D.roar20.kill_zero_usd = kz(D.roar20.tvl_usd); }
    // NFTs: the last sale (USD at the moment it sold) against today's floor, and the mark between them
    var lastSale = null; if (eps) eps.forEach(function (e) { if (e.kind === 'sale' && isNum(e.usd) && (!lastSale || Date.parse(e.ts) > Date.parse(lastSale.ts))) lastSale = { usd: e.usd / (isNum(e.count) && e.count > 1 ? e.count : 1), ts: e.ts, token: e.token || null, venue: e.venue || null }; });
    var anLast = get(S.col_analytics, 'last_sale'); if (!lastSale && anLast && isNum(anLast.notional_usd)) lastSale = { usd: anLast.notional_usd, ts: anLast.date, token: anLast.token_id || null };
    var nftFacts = function (o, last, reason) { o.last_sale_usd = last ? last.usd : null; o.last_sale_at = last ? last.ts : null; o.last_sale_reason = reason;
      o.last_vs_floor_pct = last && isNum(o.floor_usd) && last.usd > 0 ? (o.floor_usd / last.usd - 1) * 100 : null;
      o.mark_usd = last && isNum(o.floor_usd) ? (o.floor_usd + last.usd) / 2 : null; o.mark_reason = !isNum(o.floor_usd) ? 'no floor today' : reason;
      o.mcap_usd = isNum(o.floor_usd) && isNum(o.minted) ? o.floor_usd * o.minted : null;
      o.last_note = last ? (new Date(last.ts).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' }) + (last.token ? ' · #' + last.token : '') + (o.last_vs_floor_pct != null ? ' · floor ' + (o.last_vs_floor_pct >= 0 ? '+' : '−') + Math.abs(o.last_vs_floor_pct).toFixed(o.last_vs_floor_pct > -10 && o.last_vs_floor_pct < 10 ? 1 : 0) + '%' : '')) : null; };
    nftFacts(D.pl, lastSale, 'no sale in the activity ledger or the analytics');
    D.bl.minted = D.bl.minted != null ? D.bl.minted : null; nftFacts(D.bl, null, 'no Burning Lions sale recorded yet — its sales ledger (nft-flows) is not built');
    if (D.bl.listing_rows && D.bl.listing_rows.length) { if (!D.listings) D.listings = { rows: [], count: 0, built_at: null, partial: 'the pixeLions bundle did not load' }; D.listings.rows = D.listings.rows.concat(D.bl.listing_rows).sort(function (a, c) { return a.usd - c.usd; }); D.listings.count = D.listings.rows.length; }   // 1.8.0
  ...
  var CSS = [
    ...
    '.ht-mkc-v{font-weight:700;font-size:1.25rem;color:var(--ht-accent);margin:.3rem 0 .2rem;font-variant-numeric:tabular-nums;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ht-mkc-s{display:flex;justify-content:space-between;gap:.5rem;font-size:.72rem;color:var(--ht-muted)}.ht-mkc-s b{color:#fff}.ht-mkc-src{font-size:.62rem;color:var(--ht-dimmer);margin-top:.35rem}.ht-trend{display:inline-flex;align-items:center;justify-content:center;margin-left:.4rem;vertical-align:middle;width:1.35rem;height:1.2rem;border-radius:.35rem;border:1px solid var(--ht-border);background:rgba(255,255,255,.04);color:var(--ht-muted);cursor:pointer;padding:0}.ht-trend:hover{color:var(--ht-accent);border-color:var(--ht-accent)}.ht-trend-r{float:right;margin:0 0 0 .4rem}.ht-mkc-h .ht-trend{margin-left:auto}.ht-mkc-h small+.ht-trend{margin-left:.4rem}.ht-trend-m{width:min(46rem,100%)}.ht-trend-tip{position:absolute;top:4px;background:#0d0d0d;border:1px solid var(--ht-border);border-radius:8px;padding:.35rem .5rem;font-size:.7rem;color:#e5e7eb;pointer-events:none;min-width:150px}.ht-trend-tip i{display:inline-block;width:.55rem;height:.55rem;border-radius:2px;margin-right:.3rem}.ht-mkfs{border-top:1px solid var(--ht-border);margin-top:.5rem;padding-top:.4rem;display:grid;gap:.18rem}.ht-mkf{display:flex;justify-content:space-between;align-items:baseline;gap:.5rem;font-size:.7rem;color:var(--ht-muted);min-width:0}.ht-mkf>span{white-space:nowrap}.ht-mkf>b{font-weight:600;color:#e5e7eb;font-variant-numeric:tabular-nums;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:right}.ht-mkf small{font-weight:400;color:var(--ht-dimmer);font-size:.62rem}',
  ...
  // ---- 1.9.0: TRENDS. A tile names its series (trend: {src, label, fmt, lines?, note?}); src is one of
  //   nap:<SYM>        network-and-prices token_prices[SYM].prices.astroport.series  {t (s), v}         (4-hourly, about a week kept)
  //   daily:<path>     <dao>/history/daily.json days[] {day, …}                                          (daily, since 2023-09)
  //   markets:<field>  <dao>/history/markets.json days{} {…}                                             (daily, since 2026-09-26)
  //   positions:<path> <dao>/positions/daily/index.json days{} {known_usd, by_section{…}}  (several paths joined with '+' are summed)
  //   roar20:<field>   <dao>/roar20/market-history.json points[] {t (ms), …}                            (hourly)
  //   floor:<slug>     nft-collections/<slug>/snapshots/floor-history.json rows[] — the lowest listing floor across tiers
  var TREND_CACHE = {};
  function trendBtn(spec, extraCls) { if (!spec) return ''; state.trends = state.trends || []; var i = state.trends.length; state.trends.push(spec); return '<button type="button" class="ht-trend' + (extraCls ? ' ' + extraCls : '') + '" title="' + esc('chart: ' + (spec.label || 'trend')) + '" aria-label="open the trend chart" onclick="event.stopPropagation();HomeTiles.openTrend(' + i + ')"><svg viewBox="0 0 16 16" width="13" height="13" aria-hidden="true"><path d="M1 13h14M2 11l4-4 3 3 5-6" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button>'; }
  function trendSource(src) {
    var dao = M.ctx && M.ctx.tenant && (M.ctx.tenant.daos || [])[0]; var kind = String(src).split(':')[0], arg = String(src).slice(kind.length + 1);
    if (kind === 'nap') return { url: CORE + 'network-and-prices/current.json', pick: function (d) { var e = get(d, 'token_prices.' + arg + '.prices.astroport.series') || []; return e.map(function (p) { return [p.t * 1000, p.v]; }); }, label: 'network-and-prices (the TLA pool), 4-hourly' };
    if (kind === 'daily') return { url: DAOO + dao + '/history/daily.json', pick: function (d) { return (d && d.days || []).map(function (r) { return [Date.parse(r.day + 'T23:59:59Z'), get(r, arg)]; }); }, label: 'history/daily — the state at the last block of each day' };
    if (kind === 'markets') return { url: DAOO + dao + '/history/markets.json', pick: function (d) { var o = d && d.days || {}; return Object.keys(o).map(function (k) { return [Date.parse(o[k].capturedAt || k + 'T12:00:00Z'), get(o[k], arg)]; }); }, label: 'history/markets — one row a day (the day\'s last hourly run)' };
    if (kind === 'positions') return { url: DAOO + dao + '/positions/daily/index.json', pick: function (d) { var o = d && d.days || {}; return Object.keys(o).map(function (k) { var vals = arg.split('+').map(function (pth) { return get(o[k], pth); }); return [Date.parse(o[k].capturedAt || k + 'T23:00:00Z'), vals.every(function (v) { return v == null; }) ? null : sum(vals.filter(isNum))]; }); }, label: 'positions/daily — the day\'s last hourly capture' };
    if (kind === 'roar20') return { url: DAOO + dao + '/roar20/market-history.json', pick: function (d) { return (d && d.points || []).map(function (p) { return [p.t, p[arg]]; }); }, label: 'roar20/market-history — hourly' };
    if (kind === 'floor') return { url: NFTC + arg + '/snapshots/floor-history.json', pick: function (d) { return (d && d.rows || []).map(function (r) { var v = Object.keys(r.per_tier || {}).map(function (k) { return r.per_tier[k] && r.per_tier[k].listing_floor_usd; }).filter(isNum); return [Date.parse(r.capturedAt || r.date + 'T23:00:00Z'), v.length ? Math.min.apply(null, v) : null]; }); }, label: 'floor-history — the daily inventory, the lowest live listing' };
    return null;
  }
  function loadTrend(spec) { var lines = spec.lines || [{ src: spec.src, label: spec.label }];
    return Promise.all(lines.map(function (ln) { var ts = trendSource(ln.src); if (!ts) return Promise.resolve({ label: ln.label, pts: [], source: 'unknown source ' + ln.src });
      var pr = TREND_CACHE[ts.url] || (TREND_CACHE[ts.url] = jsonFetch(ts.url, { timeoutMs: 30000 }));
      return pr.then(function (d) { var pts = d ? ts.pick(d).filter(function (p) { return isFinite(p[0]) && isNum(p[1]); }).sort(function (a, b) { return a[0] - b[0]; }) : []; return { label: ln.label, pts: pts, source: ts.label, failed: !d }; }, function () { return { label: ln.label, pts: [], source: ts.label, failed: true }; }); })); }
  var TREND_COLS = ['#ffe600', '#60a5fa', '#34d399', '#f472b6'];
  function openTrend(i) {
    var spec = state.trends[i]; if (!spec) return; closeMenuModal();
    var ov = G.document.createElement('div'); ov.className = 'ht ht-modal-ov'; ov.id = 'ht-modal'; ov.onclick = function (e) { if (e.target === ov) closeMenuModal(); };   /* .ht: the modal lives on <body>, outside the themed root — the class carries the theme */
    ov.innerHTML = '<div class="ht-modal ht-trend-m" role="dialog" aria-label="' + esc(spec.label || 'trend') + '"><div class="ht-modal-h"><b>' + esc(spec.label || 'Trend') + '</b><button type="button" class="ht-x" onclick="HomeTiles.closeMenu()" aria-label="close">✕</button></div><div class="ht-modal-b" id="ht-trend-b"><div class="ht-note">Loading the series…</div></div></div>';
    G.document.body.appendChild(ov); G.document.body.style.overflow = 'hidden'; G.document.addEventListener('keydown', escClose);
    loadTrend(spec).then(function (series) { state.trendSeries = series; state.trendSpec = spec; state.trendRange = state.trendRange != null ? state.trendRange : 30; paintTrend(); });
  }
  function paintTrend() {
    var b = G.document.getElementById('ht-trend-b'); if (!b) return; var spec = state.trendSpec, series = state.trendSeries || []; var f = fmt[spec.fmt || 'num'] || fmt.num;
    var R = state.trendRange; var cut = R ? Date.now() - R * 864e5 : -Infinity;
    var vis = series.map(function (s) { return { label: s.label, source: s.source, failed: s.failed, pts: s.pts.filter(function (p) { return p[0] >= cut; }), all: s.pts }; });
    var all = [].concat.apply([], vis.map(function (s) { return s.pts; }));
    var ranges = [[7, '7d'], [30, '30d'], [90, '90d'], [0, 'All']].map(function (r) { return '<button type="button" class="ht-tab' + (state.trendRange === r[0] ? ' ht-on' : '') + '" onclick="HomeTiles.trendRange(' + r[0] + ')">' + r[1] + '</button>'; }).join('');
    var head = '<div class="ht-tabs" style="margin:.2rem 0 .6rem">' + ranges + '</div>';
    var first = vis[0] && vis[0].all; var span = first && first.length ? Math.round((first[first.length - 1][0] - first[0][0]) / 864e5) : 0;
    var srcLine = '<div class="ht-src">' + esc(vis.map(function (s) { return s.source; }).filter(function (x, j, a) { return x && a.indexOf(x) === j; }).join(' · ')) + (span ? ' · ' + span + ' day' + (span === 1 ? '' : 's') + ' kept so far' : '') + '</div>';
    if (all.length < 2) { b.innerHTML = head + '<div class="ht-note">' + (vis.some(function (s) { return s.failed; }) ? 'The series did not load — try again shortly.' : first && first.length ? 'Collecting: ' + first.length + ' point' + (first.length === 1 ? '' : 's') + ' so far, since ' + esc(fmt.date(new Date(first[0][0]).toISOString())) + '. The chart draws once this range has two — try All, or check back in a few days.' : 'No points kept for this yet — the series starts with the next capture.') + '</div>' + srcLine; return; }
    var W = 640, H = 240, P = { l: 8, r: 8, t: 10, b: 22 };
    var xs = all.map(function (p) { return p[0]; }), ys = all.map(function (p) { return p[1]; }); var x0 = Math.min.apply(null, xs), x1 = Math.max.apply(null, xs), y0 = Math.min.apply(null, ys), y1 = Math.max.apply(null, ys); if (y0 === y1) { y0 = y0 * 0.98; y1 = y1 * 1.02 || 1; } var pad = (y1 - y0) * 0.08; y0 -= pad; y1 += pad; if (x0 === x1) x1 = x0 + 1;
    var X = function (t) { return P.l + (t - x0) / (x1 - x0) * (W - P.l - P.r); }, Y = function (v) { return P.t + (1 - (v - y0) / (y1 - y0)) * (H - P.t - P.b); };
    var paths = vis.map(function (s, k) { if (s.pts.length < 1) return ''; var d = s.pts.map(function (p, j) { return (j ? 'L' : 'M') + X(p[0]).toFixed(1) + ',' + Y(p[1]).toFixed(1); }).join(''); var col = TREND_COLS[k % TREND_COLS.length];
      var area = vis.length === 1 ? '<path d="' + d + 'L' + X(s.pts[s.pts.length - 1][0]).toFixed(1) + ',' + (H - P.b) + 'L' + X(s.pts[0][0]).toFixed(1) + ',' + (H - P.b) + 'Z" fill="' + col + '" opacity=".10"/>' : '';
      return area + '<path d="' + d + '" fill="none" stroke="' + col + '" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke"/>' + (s.pts.length < 40 ? s.pts.map(function (p) { return '<circle cx="' + X(p[0]).toFixed(1) + '" cy="' + Y(p[1]).toFixed(1) + '" r="2.6" fill="' + col + '"/>'; }).join('') : ''); }).join('');
    var d0 = new Date(x0).toISOString(), d1 = new Date(x1).toISOString();
    var svg = '<svg id="ht-trend-svg" viewBox="0 0 ' + W + ' ' + H + '" width="100%" style="display:block;touch-action:pan-y"><line x1="' + P.l + '" x2="' + (W - P.r) + '" y1="' + (H - P.b) + '" y2="' + (H - P.b) + '" stroke="#333"/>' + paths + '<line id="ht-trend-x" x1="0" x2="0" y1="' + P.t + '" y2="' + (H - P.b) + '" stroke="#888" stroke-dasharray="3 3" visibility="hidden"/><text x="' + P.l + '" y="' + (H - 6) + '" fill="#777" font-size="11">' + esc(fmt.date(d0)) + '</text><text x="' + (W - P.r) + '" y="' + (H - 6) + '" fill="#777" font-size="11" text-anchor="end">' + esc(fmt.date(d1)) + '</text></svg>';
    var stats = vis.map(function (s, k) { if (!s.pts.length) return '<div class="ht-lg"><i style="background:' + TREND_COLS[k % 4] + '"></i><span class="ht-lg-l">' + esc(s.label || '') + '</span><span class="ht-lg-v">no points in this range</span></div>'; var a = s.pts[0][1], z = s.pts[s.pts.length - 1][1]; var ch = a ? (z / a - 1) * 100 : null; var mn = Math.min.apply(null, s.pts.map(function (p) { return p[1]; })), mx = Math.max.apply(null, s.pts.map(function (p) { return p[1]; }));
      return '<div class="ht-lg"><i style="background:' + TREND_COLS[k % 4] + '"></i><span class="ht-lg-l">' + esc(s.label || '') + ' <b style="color:#fff">' + esc(f(z)) + '</b></span><span class="ht-lg-p ' + (ch > 0 ? 'ht-up' : ch < 0 ? 'ht-dn' : '') + '">' + (ch == null ? '' : (ch >= 0 ? '+' : '') + ch.toFixed(Math.abs(ch) < 10 ? 2 : 1) + '%') + '</span><span class="ht-lg-v">low ' + esc(f(mn)) + ' · high ' + esc(f(mx)) + ' · ' + s.pts.length + ' pts</span></div>'; }).join('');
    b.innerHTML = head + '<div style="position:relative">' + svg + '<div id="ht-trend-tip" class="ht-trend-tip" style="display:none"></div></div><div class="ht-legend" style="margin-top:.5rem">' + stats + '</div>' + (spec.note ? '<div class="ht-s" style="margin-top:.4rem">' + esc(spec.note) + '</div>' : '') + srcLine;
    var el = G.document.getElementById('ht-trend-svg'), tip = G.document.getElementById('ht-trend-tip'), xl = G.document.getElementById('ht-trend-x');
    var move = function (ev) { var r = el.getBoundingClientRect(); var cx = ((ev.touches ? ev.touches[0].clientX : ev.clientX) - r.left) / r.width * W; var t = x0 + (cx - P.l) / (W - P.l - P.r) * (x1 - x0);
      var lines = vis.map(function (s, k) { if (!s.pts.length) return null; var best = s.pts.reduce(function (a, p) { return Math.abs(p[0] - t) < Math.abs(a[0] - t) ? p : a; }); return { k: k, p: best, label: s.label }; }).filter(Boolean); if (!lines.length) return;
      var px = X(lines[0].p[0]); xl.setAttribute('x1', px); xl.setAttribute('x2', px); xl.setAttribute('visibility', 'visible');
      tip.style.display = 'block'; tip.style.left = Math.min(r.width - 170, Math.max(0, px / W * r.width - 80)) + 'px'; tip.innerHTML = '<b>' + esc(new Date(lines[0].p[0]).toISOString().slice(0, 16).replace('T', ' ')) + ' UTC</b>' + lines.map(function (l) { return '<div><i style="background:' + TREND_COLS[l.k % 4] + '"></i>' + esc(l.label || '') + ' ' + esc(f(l.p[1])) + '</div>'; }).join(''); };
    el.addEventListener('mousemove', move); el.addEventListener('touchmove', move, { passive: true }); el.addEventListener('mouseleave', function () { tip.style.display = 'none'; xl.setAttribute('visibility', 'hidden'); });
  }
  function trendRange(r) { state.trendRange = r; paintTrend(); }
  function metricTile(m, D) {
    ...
    return '<div class="ht-tile"' + href + '><div class="ht-k">' + esc(m.label) + trendBtn(m.trend) + '</div><div class="ht-v' + (shown && shown.length > 12 ? ' ht-v-sm' : '') + '">' + (shown == null ? unk(m.unknown || (m.reason_key && get(D, m.reason_key)) || m.reason) : esc(shown)) + '</div>' + (sub ? '<div class="ht-s">' + sub + '</div>' : '') + (m.source ? '<div class="ht-src">' + (/\{[a-z0-9_.|]+\}/i.test(m.source) ? fill(m.source, D) : esc(m.source)) + '</div>' : '') + '</div>';
  }
  ...
  function renderMarkets(sec, D) {
    // 1.7.0: the fact rows — every card gets the same count (sec.fact_rows), a missing figure is a dim dash with its reason on hover
    var facts = function (it) { var rows = it.facts || []; var n = Math.max(rows.length, sec.fact_rows || 0); if (!n) return ''; var h = '';
      for (var i = 0; i < n; i++) { var f = rows[i]; if (!f) { h += '<div class="ht-mkf"><span>&nbsp;</span><b></b></div>'; continue; }
        var v = get(D, f.key); var ff = fmt[f.fmt || 'usd']; var note = f.note_key ? get(D, f.note_key) : null; var why = f.reason_key ? get(D, f.reason_key) : f.reason;
        h += '<div class="ht-mkf" title="' + esc((f.help || '') + (note ? (f.help ? ' — ' : '') + note : '')) + '"><span>' + esc(f.label) + '</span>' + (v == null ? '<b class="ht-mk-na" title="' + esc(why || 'not read') + '">—</b>' : '<b>' + esc(ff ? ff(v) : String(v)) + (note && f.inline_note ? ' <small>' + esc(note) + '</small>' : '') + '</b>') + '</div>'; }
      return '<div class="ht-mkfs">' + h + '</div>'; };
    var cards = (sec.items || []).map(function (it) {
      ...
      if (v == null && it.value_fallback_key) { var fb = get(D, it.value_fallback_key); if (fb != null) shown = String(fb); }   /* 1.6.3 */
      ...
      return '<div class="ht-mkc' + (it.href ? ' ht-link' : '') + '"' + go + '><div class="ht-mkc-h">' + (it.logo ? '<img src="' + esc(it.logo) + '" alt="" onerror="this.remove()">' : '') + '<span>' + esc(it.label) + '</span>' + ((it.what_key && get(D, it.what_key)) || it.what ? '<small>' + esc((it.what_key && get(D, it.what_key)) || it.what) + '</small>' : '') + trendBtn(it.trend) + '</div>' +
        '<div class="ht-mkc-v">' + (shown == null ? unk(reason) : esc(shown)) + '</div><div class="ht-mkc-s">' + chg + volTxt + '</div>' + facts(it) + (it.source ? '<div class="ht-mkc-src">' + esc(it.source) + '</div>' : '') + '</div>';
    }).join('');
  ...
  function renderSupply(sec, D) {
    var tiles = (sec.bars || []).map(function (b) {
      ...
      var tsrc = b.total_source_key ? get(D, b.total_source_key) : null; return '<div class="ht-tile"><div class="ht-k">' + trendBtn(b.trend, 'ht-trend-r') + esc(b.label) + (b.total_key ? ' · of <b>' + (total == null ? unk(b.total_reason) : esc((b.total_fmt ? fmt[b.total_fmt] : fmt.big)(total))) + '</b>' + (tsrc && /^(registry|product)/.test(String(tsrc)) ? ' <small class="ht-dim" title="the total is the registry\'s figure; the live read did not answer">' + esc(tsrc) + '</small>' : '') : '') + '</div>' + (b.lead ? '<div class="ht-lead">' + fill(b.lead, D) + '</div>' : '') + bar(parts, total, D, { hasTotal: !!b.total_key, restLabel: b.rest_label, fmt: b.total_fmt ? fmt[b.total_fmt] : null }) + (b.note ? '<details class="ht-how"><summary>how it is measured</summary><div class="ht-src">' + fill(b.note, D) + '</div></details>' : '') + ((b.links || []).length ? '<div class="ht-src" style="margin-top:.35rem">' + b.links.map(function (l) { return '<a href="' + esc(l.href) + '" style="color:var(--ht-accent)">' + esc(l.label) + ' →</a>'; }).join(' · ') + '</div>' : '') + '</div>';
    }).join('');
  ...
  function renderRow(sec, D) {
    ...
    if (sec.total) { var parts = (sec.total.parts_key ? (get(D, sec.total.parts_key) || []) : (sec.total.parts || []).map(function (p) { return { label: p.label, usd: get(D, p.key), note: p.reason }; }));
      ...
      var nosum = sec.total.sum === false;   /* 1.9.0: figures that do not add up to anything (a market cap and a volume) sit side by side */
      total = '<div class="ht-total"' + (sec.total.href ? ' onclick="location.href=\'' + esc(sec.total.href) + '\'" style="cursor:pointer" role="link" tabindex="0"' : '') + '><span class="ht-tt">' + esc(sec.total.label) + trendBtn(sec.total.trend) + '</span>' + parts.map(function (p) { return '<span>' + esc(p.label) + ' <b>' + (p.usd == null ? unk(p.note) : esc(fmt.usd(p.usd))) + '</b></span>'; }).join(nosum ? '<span>·</span>' : '<span>+</span>') + (nosum ? '' : '<span>=</span><span class="ht-tsum">' + (known == null ? unk() : esc(fmt.usd(known))) + '</span>' + (parts.some(function (p) { return p.usd == null; }) ? '<span>known parts only</span>' : '')) + '</div>'; }
  ...
  function renderRewards(sec, D) {
    var tiles = (sec.groups || []).map(function (g) {
      ...
      return '<div class="ht-tile"' + open + '><div class="ht-k">' + esc(g.label) + trendBtn(g.trend) + (g.href ? ' <span style="float:right;color:var(--ht-accent);font-size:.7rem">open →</span>' : '') + '</div><div class="ht-v">' + (v == null ? unk(g.reason) : esc(fmt.usd(v))) + '</div>' + lines + (g.source ? '<div class="ht-src">' + esc(g.source) + '</div>' : '') + '</div>';
    }).join('');
  ...
  function colFor(ctx, r) { var s = r && r.col; if (!s) return ctx.primary; return (ctx.collections || []).filter(function (c) { return c.slug === s; })[0] || ctx.primary; }   // 1.8.0
  function rowName(ctx, r) { var c = colFor(ctx, r); var t = c && c.media && c.media.tokens && c.media.tokens[String(r.id)]; if (t && t.name) return t.name; return c && c.token_name ? c.token_name(r.id) : tokenName(ctx, r.id); }
  ...
  function classOf(r, classes) { var hit = classes.filter(function (c) { return c.rule && (c.rule === 'collection' ? r.col === c.collection : r.col ? false : c.rule === 'rank1' ? r.rank === 1 : c.rule === 'base' ? r.rank !== 1 : false); })[0]; return hit ? hit.label : null; }   /* 1.8.0: a collection class; tiers apply to the primary's rows */
  ...
  function renderMarket(sec, D, ctx) {
    ...
    var tabs = (sec.collection_tabs || []).map(function (t) { var tag = t.href && !t.disabled ? 'a' : 'span'; return '<' + tag + (tag === 'a' ? ' href="' + esc(t.href) + '"' : '') + ' class="ht-ctab' + (t.active ? ' ht-on' : '') + (t.disabled ? ' ht-off' : '') + '" title="' + esc(t.reason || '') + '" style="text-decoration:none">' + (t.icon ? '<i class="fas ' + esc(t.icon) + '"></i>' : '') + esc(t.label) + '</' + tag + '>'; }).join('');   /* 1.8.0: a tab with href opens that collection */
    ...
    return '<section class="ht-sec" id="ht-market"><div class="ht-h ht-mk-top"><span><i class="fas fa-store ht-c"></i> ' + esc(sec.title || '') + (sec.sub ? '<small>' + fill(sec.sub, D) + '</small>' : '') + '</span><span class="ht-ctabs">' + tabs + '</span></div>' +
      ...
      '<div class="ht-tile ht-mk" style="margin-top:.9rem"><div class="ht-mk-h"><span class="ht-mk-t"><i class="fas fa-tags ht-y"></i> ' + esc(sec.listings_title || 'All Current Listings') + ' <small id="ht-list-count"></small></span><span class="ht-src" style="margin:0">Show: <span class="ht-ctab ht-on">' + esc((ctx.collections && ctx.collections.length ? ctx.collections : [ctx.primary]).filter(Boolean).map(function (c) { return c.label; }).join(' + ')) + '</span></span></div><div id="ht-list-ctl" class="ht-tabs"></div><div id="ht-list" class="ht-lgrid ht-scroll"></div><div id="ht-list-more" class="ht-center"></div></div>' +
  ...
  function paintListings(sec, D, ctx) {
    ...
    el.innerHTML = shown.map(function (r) { var rc = colFor(ctx, r); var u = null; try { u = rc && rc.assets && rc.assets.image ? rc.assets.image(r.id) : null; } catch (e) { u = null; } var cl = classOf(r, classes); var f = get(D, 'pl.floor_' + r.venue); return '<a class="ht-lcard" href="' + esc(explorer + (explorer.indexOf('?') === -1 ? '?' : '&') + (r.col ? 'collection=' + encodeURIComponent(r.col) + '&' : '') + 'search=' + encodeURIComponent(r.id)) + '">' + (u ? '<img loading="lazy" src="' + esc(u) + '" alt="">' : '<span class="ht-lc-img"></span>') + '<span class="ht-lc-b"><span class="ht-lc-n">' + esc(rowName(ctx, r)) + (own ? ' <span class="ht-chip ht-chip-o">' + esc(own) + '</span>' : '') + '</span><span class="ht-lc-s">' + (cl ? '<span class="ht-y">' + esc(cl) + '</span> · ' : '') + (r.rank != null ? 'Rank #' + esc(fmt.int(r.rank)) + ' · ' : '') + esc(VENUE_SHORT[r.venue] || 'venue?') + (r.chain_only ? ' · chain-only' : '') + '</span></span><span class="ht-lc-p"><b class="ht-g">' + esc(fmt.usd(r.usd, 2)) + '</b><small>' + (f && f.symbol ? 'listed in ' + esc(f.symbol) : '') + '</small></span></a>'; }).join('') || '<div class="ht-note" style="grid-column:1/-1">Nothing listed in this class.</div>';
  ...
  function paintAll() {
    state.trends = [];   /* 1.9.0: the chart buttons re-register on every paint */
  ...
  return { openTrend: openTrend, trendRange: trendRange, VERSION: VERSION, mount: mount, derive: derive, load: loadSources, get: get, fmt: fmt, fill: fill, CSS: CSS, openSheet: openSheet, closeSheets: closeSheets, toggleMenu: toggleMenu, closeMenu: closeMenuModal, toggleSales: toggleSales, addr: addr, setWallet: setWallet, setList: setList, setAct: setAct, _state: state, _M: M };
});
