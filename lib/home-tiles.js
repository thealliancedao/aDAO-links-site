...
(function (root, factory) {
  ...
  'use strict';
  var VERSION = '1.2.1';   // 1.2.1 (2026-09-22, owner's second look): the chip says Coming; the hero is a mane (rings of rays + the tenant's logo medallion), no caution tape; bars fall back to the registry's fixed supplies, labeled. 1.2.0 (2026-09-22): the market section in aDAO's exact shape (three venue cards · Live Activity full width with kind chips · All Current Listings grid with venue + class filters · Top 10 behind a button); sales-enriched folded per venue; pyROAR + ROAR20 supply bars (live reads: pyROAR token_info + roster balances; Solana getTokenSupply + getTokenLargestAccounts). 1.1.0 (2026-09-22, home v2): the positions PRODUCT (dao-originations/<dao>/positions, ally-positions) replaces the live roster block for tla / unclaimed / treasury / total when it loads (live LCD stays the fallback, source-labeled); rewards groups take href (a full page, no sheet); loadSources exported as load() for the tenant's other pages · 1.0.2: burn.supply_delta (initial − live supply, labeled) · 1.0.1: tenant.roar20 (DexScreener, labeled) + tenant.burn (reported figures, source-labeled) pass through to the tiles
  ...
  function renderSupply(sec, D) {
    var tiles = (sec.bars || []).map(function (b) {
      ...
      var tsrc = b.total_source_key ? get(D, b.total_source_key) : null; return '<div class="ht-tile"><div class="ht-k">' + esc(b.label) + (b.total_key ? ' · of <b>' + (total == null ? unk(b.total_reason) : esc((b.total_fmt ? fmt[b.total_fmt] : fmt.big)(total))) + '</b>' + (tsrc && tsrc !== 'live' ? ' <small class="ht-dim" title="the total is the registry\'s figure; the live read did not answer">' + esc(tsrc) + '</small>' : '') : '') + '</div>' + bar(parts, total, D, { hasTotal: !!b.total_key }) + (b.note ? '<div class="ht-src">' + fill(b.note, D) + '</div>' : '') + '</div>';
    }).join('');
  ...
  return { VERSION: VERSION, mount: mount, derive: derive, load: loadSources, get: get, fmt: fmt, fill: fill, CSS: CSS, openSheet: openSheet, closeSheets: closeSheets, toggleMenu: toggleMenu, toggleSales: toggleSales, setList: setList, setAct: setAct, _state: state, _M: M };
});
