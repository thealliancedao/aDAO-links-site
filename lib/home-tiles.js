...
(function (root, factory) {
  ...
  function loadSources(cfg, ctx, onSource) {
    ...
    if (st.roar_cw20 && st.amproar_hub) { J('lcd_hub_roar', smart(lcd, st.roar_cw20, { balance: { address: st.amproar_hub } })); if (st.roar_staking) J('lcd_hub_staked', smart(lcd, st.roar_staking, { staked_balance: { address: st.amproar_hub } })); }
  ...
  function derive(S, cfg, ctx) {
    ...
    D.roar.hub_balance = S.lcd_hub_roar && S.lcd_hub_roar.balance !== undefined ? human(S.lcd_hub_roar.balance) : null; D.roar.hub_staked = S.lcd_hub_staked && S.lcd_hub_staked.balance !== undefined ? human(S.lcd_hub_staked.balance) : null;
  ...
  function walletReads(addr) {
    ...
    var reads = {
      ...
      staked: st.roar_staking ? smart(lcd, st.roar_staking, { staked_balance: { address: addr } }) : Promise.resolve(null),
  ...
  function toggleMenu(tile) { var m = tile.querySelector('.ht-menu'); if (!m) return; closeMenuModal(); var title = tile.querySelector('.ht-nt') ? tile.querySelector('.ht-nt').textContent.replace(/[▾▼]/g, '').trim() : ''; var ov = G.document.createElement('div'); ov.className = 'ht-modal-ov'; ov.id = 'ht-modal'; ov.innerHTML = '<div class="ht-modal" role="dialog" aria-label="' + esc(title) + '"><div class="ht-modal-h"><b>' + esc(title) + '</b><button type="button" class="ht-x" onclick="HomeTiles.closeMenu()" aria-label="close">✕</button></div><div class="ht-modal-b">' + m.innerHTML + '</div></div>'; ov.addEventListener('click', function (ev) { if (ev.target === ov) closeMenuModal(); }); G.document.body.appendChild(ov); G.document.addEventListener('keydown', escClose); }
  function escClose(ev) { if (ev.key === 'Escape') closeMenuModal(); }
  function closeMenuModal() { var ov = G.document.getElementById('ht-modal'); if (ov) ov.remove(); G.document.removeEventListener('keydown', escClose); }
