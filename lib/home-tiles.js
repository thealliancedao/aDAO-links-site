...
(function (root, factory) {
  ...
  'use strict';
  var VERSION = '1.6.1';   // 1.6.1: ROAR20's market from the hourly capture (ally-positions 1.3.0 → <dao>/roar20/market.json) — the browser's DexScreener read is now only the override; pyROAR's logo and the burn wording follow the festival's published mechanics.
  ...
  function derive(S, cfg, ctx) {
    ...
    D.bl.market_reason = 'the collection is not onboarded yet (nft-collections/burning-lions) — floor and sales land with it';
  ...
  function renderMarkets(sec, D) {
    var cards = (sec.items || []).map(function (it) {
      ...
      var chg = c == null ? '<span class="ht-mk-na" title="' + esc(mreason || it.chg_reason || 'no 24h figure') + '">24h —</span>' : '<span class="' + (c > 0 ? 'ht-up' : c < 0 ? 'ht-dn' : 'ht-dim') + '">' + esc(fmt.pct(c)) + ' <small>24h</small></span>';
      var volTxt = vol == null ? '<span class="ht-mk-na" title="' + esc(mreason || it.vol_reason || 'no volume figure') + '">' + esc(it.vol_label || 'vol 24h') + ' —</span>' : '<span>' + esc(it.vol_label || 'vol 24h') + ' <b>' + esc(volf ? volf(vol) : String(vol)) + '</b></span>';
