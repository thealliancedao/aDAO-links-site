...
(function (root, factory) {
  ...
  function loadSources(cfg, ctx, onSource) {
    ...
    if (T.burning_lions && T.burning_lions.contract) { J('lcd_bl_num_tokens', smart(lcd, T.burning_lions.contract, { num_tokens: {} })); J('lcd_bl_info', smart(lcd, T.burning_lions.contract, { contract_info: {} }));
      ...
      J('bl_floor', jsonFetch(NFTC + 'burning-lions/snapshots/floor-history.json'));   /* 1.6.3: the collection's own inventory (a 404 until its first run) */
  ...
  function renderMarkets(sec, D) {
    ...
    var cards = (sec.items || []).map(function (it) {
      ...
      return '<div class="ht-mkc' + (it.href ? ' ht-link' : '') + '"' + go + '><div class="ht-mkc-h">' + (it.logo ? '<img src="' + esc(it.logo) + '" alt="" onerror="this.remove()">' : '') + '<span>' + esc(it.label) + '</span>' + ((it.what_key && get(D, it.what_key)) || it.what ? '<small>' + esc((it.what_key && get(D, it.what_key)) || it.what) + '</small>' : '') + '</div>' +
