...
(function (root) {
  'use strict';
  const VERSION = '2.1.0';   // 2.1.0 (owner 2026-09-26: "why does the top prop have more details — should the others have the same?"): every prop card decodes its own messages when the capture has not (base64 wasm msgs → action + args, contract names from the catalog when the page passes nameOf), so a prop the corpus has not caught yet (CAPA #16) or a DAO outside the corpus (ampCAPA) reads like the rest. 2.0.0 (owner, 2026-09-24): PROPS ONLY on the home window — the Ecosystem tile is off (opts.ecosystem brings it back; too much to maintain);
  ...
  // 2.1.0: decode a proposal's raw messages in the browser — wasm execute / instantiate / migrate (base64 or object msg) and bank
  //   sends; anything else passes through as it came. Amounts stay raw (no decimals are guessed); names come from opts.nameOf.
  function decodeMsgs(raw, opts) {
    const b64 = (m) => { if (m && typeof m === 'object') return m; try { return JSON.parse(decodeURIComponent(escape(atob(String(m))))); } catch (e) { return { undecodable: String(m).slice(0, 200) }; } };
    const nm = (a) => { try { return opts && opts.nameOf && a ? opts.nameOf(a) : null; } catch (e) { return null; } };
    return (raw || []).map((m) => {
      const w = m && (m.wasm || (m['@type'] === '/cosmwasm.wasm.v1.MsgExecuteContract' ? { execute: { contract_addr: m.contract, msg: m.msg, funds: m.funds } } : null));
      if (w && w.execute) { const body = b64(w.execute.msg); const action = body && typeof body === 'object' ? Object.keys(body)[0] : null; return { type: 'wasm.execute', contract: w.execute.contract_addr, contractName: nm(w.execute.contract_addr), action, args: action ? body[action] : body, funds: w.execute.funds || [] }; }
      if (w && w.instantiate) return { type: 'wasm.instantiate', code_id: w.instantiate.code_id, label: w.instantiate.label, admin: w.instantiate.admin || null, args: b64(w.instantiate.msg), funds: w.instantiate.funds || [] };
      if (w && w.migrate) return { type: 'wasm.migrate', contract: w.migrate.contract_addr, contractName: nm(w.migrate.contract_addr), new_code_id: w.migrate.new_code_id, args: b64(w.migrate.msg) };
      if (w && (w.update_admin || w.clear_admin)) return { type: w.update_admin ? 'wasm.update_admin' : 'wasm.clear_admin', contract: (w.update_admin || w.clear_admin).contract_addr, admin: w.update_admin ? w.update_admin.admin : null };
      const bank = m && m.bank && m.bank.send; if (bank) return { type: 'bank.send', to: bank.to_address, toName: nm(bank.to_address), amount: bank.amount };
      return m;
    });
  }
  ...
  root.AlertCenter = { VERSION, RULES, build, render, openFull, itemCard, itemInline, marker, decodeMsgs, _ago: ago };
