...
(function (root) {
  ...
  function build(inputs, opts) {
    ...
    const taRow = (r) => row({ ts: Date.parse(r.ts), label: r.label, value: r.value, link: 'tla-stats.html', linkLabel: 'TLA stats', rule: 'tla:' + r.rule, source: 'member-data/tla-alerts (docs/curated/alert-thresholds.json)', raw: r.raw, data: { tla: r } });
    ...
    // --- Ecosystem ---------------------------------------------------------------
    {
      ...
      tiles.push({ key: 'ecosystem', title: 'Ecosystem', counters: [
        counter('tla', 'TLA', pick('gauge_set_change', 'epoch_flip'), taRows ? null : 'member-data/tla-alerts not published yet'),
    ...
    // --- TLA (member-data/tla-alerts product; rules + thresholds in docs/curated/alert-thresholds.json) ----------
    {
      ...
      tiles.push({ key: 'tla', title: 'TLA', counters: [counter('vp', 'VP moves', pick('bucket_vp_move', 'pool_vp_move'), gap), counter('liq', 'Liquidity · Volume', pick('pool_liquidity_move', 'pool_volume_spike'), gap), counter('apr', 'APR', pick('pool_apr_move'), gap)] });
    }
    // --- Props -------------------------------------------------------------------
    {
    ...
    // --- NFTs aDAO ---------------------------------------------------------------
    {
      ...
      if (fh) for (const tier of Object.keys(fh)) { const s = (fh[tier] || []).filter(p => num(p.floor_usd) != null); if (s.length < 2) continue; const a = s[s.length - 2], b = s[s.length - 1]; const ts = Date.parse(b.date); if (!inWin(ts)) continue; const pct = (b.floor_usd - a.floor_usd) / a.floor_usd * 100; if (pct <= -R.FLOOR_DROP_PCT) drops.push(row({ ts, label: `${tier} floor down ${Math.abs(pct).toFixed(0)}%`, value: `$${a.floor_usd.toFixed(2)} → $${b.floor_usd.toFixed(2)}`, link: explorer, linkLabel: 'explorer', rule: `nft:floor_drop≥${R.FLOOR_DROP_PCT}%`, source: 'floor-history', raw: { tier, from: a.floor_usd, to: b.floor_usd } })); }
      ...
      const staking = [];
      if (massN >= R.MASS_UNSTAKE_N || (massPct != null && massPct >= R.MASS_UNSTAKE_PCT)) staking.push(row({ ts: unst[0].ts, label: `Mass unstake: ${massN} NFTs in 24h`, value: massPct != null ? `${massPct.toFixed(2)}% of staked` : '', link: '#live-activity', linkLabel: 'feed', rule: `nft:mass_unstake≥${R.MASS_UNSTAKE_N}|${R.MASS_UNSTAKE_PCT}%`, source: 'ledger', raw: { n: massN, pct: massPct }, mine: unst.some(x => mine(x.token)), data: { tokens: unst.map(x => x.token) } }));
      ...
      const p2pRows = [];
      for (const [from, xs] of Object.entries(bySender)) if (xs.length >= R.MASS_TRANSFER_N) p2pRows.push(row({ ts: xs[0].ts, label: `Mass transfer: ${xs.length} NFTs from ${from.slice(0, 12)}…`, value: '', link: `https://chainsco.pe/terra2/address/${from}`, linkLabel: 'wallet', rule: `nft:mass_transfer≥${R.MASS_TRANSFER_N}`, source: 'ledger / transfers', raw: { from, n: xs.length }, mine: xs.some(x => mine(x.token)), data: { tokens: xs.map(x => x.token), from } }));
