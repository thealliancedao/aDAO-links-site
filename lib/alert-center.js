...
(function (root) {
  'use strict';
  const VERSION = '1.3.0';   // 1.3.0: NFT cards — both parties with holdings, token status, USD then/now + spread; Staking / Breaks   // 1.2.0: full text + links on registry cards, marketplace counts every venue event (notable first), props from the corpus with full description / decoded messages / Quick audit
  ...
  function nftCard(r, now, opts) {
    ...
    return `<div class="ac-card ${r.mine ? 'mine' : ''}"><div class="ac-nft">${img ? `<img src="${esc(img)}" alt="#${esc(tok)}" loading="lazy" onerror="this.style.visibility='hidden'">` : '<div></div>'}<div>
      <div class="h"><div class="ttl">${r.mine ? '<span class="ac-pill red">yours</span>' : ''}${r.notable ? '<span class="ac-pill amber">notable</span>' : ''}${esc(r.label)}</div><div class="meta">${r.ts ? esc(ago(r.ts, now)) : ''}</div></div>
