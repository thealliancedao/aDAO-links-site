...
(function (root) {
  ...
  function build(inputs, opts) {
    ...
    // --- Props -------------------------------------------------------------------
    {
      ...
      // 1.10.0 (owner, PD A265/A266): a prop that reached threshold and EXECUTED while its voting window is still open never
      // closed a vote inside any window — it fell through 'live' (no longer open) and 'executed' (vote end in the future).
      // It is the thing a council most wants to see, so it is an ALERT of its own, counted while its vote is still open.
      const early = cards.filter(c => !isOpen(c) && done(c) && c.endIso && Date.parse(c.endIso) > now).map(c => { const left = (Date.parse(c.endIso) - now) / D; const r = prow(c, 'prop:executed_early'); r.severity = 'alert'; r.value = `EXECUTED with ${left >= 1 ? Math.round(left) + ' day' + (Math.round(left) === 1 ? '' : 's') : Math.max(1, Math.round(left * 24)) + ' h'} of voting left — passed its threshold and ran before the vote closed` + (c.yesPct != null ? ` · yes ${Number(c.yesPct).toFixed(0)}%` : '') + (c.turnoutPct != null ? ` · turnout ${Number(c.turnoutPct).toFixed(0)}%` : ''); r.ts = c.executedIso ? Date.parse(c.executedIso) : (c.startIso ? Date.parse(c.startIso) : now); r.notable = true; return r; });
      const exec = cards.filter(c => !isOpen(c) && done(c) && c.endIso && inWin(Date.parse(c.endIso))).map(c => prow(c, 'prop:executed(voting_end_in_window)'));
      tiles.push({ key: 'props', title: 'Props', counters: [counter('live', 'Live', live), counter('early', 'Executed early', early, early.length ? null : 'a prop that executes before its vote closes shows here while the vote is still open'), counter('veto', 'Veto lock', veto), counter('exec', 'Executed', exec, 'the capture records when voting ended, not when the executor fired — a prop counts here when its vote closed inside the window')], stateOnly: ['live', 'early', 'veto'] });
    }
    // --- NFTs aDAO ---------------------------------------------------------------
