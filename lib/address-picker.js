...
(function (root, doc) {
  ...
  var CSS = [
    ...
    '.ap-menu{position:absolute;top:calc(100% + .4rem);right:0;width:min(24rem,92vw);max-height:60vh;overflow:auto;background:#0f1117;border:1px solid rgba(34,211,238,.25);border-radius:.75rem;box-shadow:0 20px 40px rgba(0,0,0,.5);padding:.35rem;z-index:60;display:none}',
    ...
    '@media(max-width:767px){.ap-pill{min-width:0;max-width:11rem}.ap-menu{right:-3rem}}',
  ].join('\n');
  ...
  function open() { state.open = true; state.els.menu.classList.add('open'); state.els.pill.classList.add('open'); }
