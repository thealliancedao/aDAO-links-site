 ...
 * 2026-09-23 (owner): the cyan is the DEFAULT accent — a tenant page that sets --t-accent (Lion DAO: #ffe600) themes the picker; aDAO pages set none and keep cyan (every color falls back to what it was).
...
(function (root, doc) {
  ...
  var CSS = [
    ...
    '@media (min-width:768px){.ap-pill{max-width:44em}.ap-pill .ap-in{font-size:.85em}}',
    '.ap-pill:focus-within,.ap-pill.open{border-color:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 60%,transparent);box-shadow:0 0 0 3px color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent)}',
    '.ap-pill i.fa-magnifying-glass{color:var(--t-accent,#67e8f9);font-size:.8em}',
    ...
    '.ap-copy{flex:0 0 auto;width:1.6em;height:1.6em;border-radius:999px;border:0;background:transparent;color:#94a3b8;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:.75em}',
    '.ap-copy:hover{color:var(--t-accent,#67e8f9);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 10%,transparent)}.ap-copy.ok{color:#6ee7b7}',
    '.ap-x:focus-visible,.ap-save:focus-visible,.ap-row:focus-visible{outline:2px solid var(--t-accent,#67e8f9);outline-offset:1px}',
    '.ap-menu{position:fixed;width:min(24em,92vw);max-height:60vh;overflow:auto;background:#0f1117;border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 25%,transparent);border-radius:.75em;box-shadow:0 20px 40px rgba(0,0,0,.5);padding:.35em;z-index:9000;display:none}',
    ...
    '.ap-row:hover,.ap-row.hi{background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 10%,transparent)}',
    '.ap-row .ap-nm{font-weight:600;font-size:.85em;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
    '.ap-row .ap-nm b{color:var(--t-accent,#67e8f9);font-weight:700}',
    ...
    '.ap-chip.adao{color:var(--t-accent,#67e8f9);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent)}.ap-chip.liondao{color:#fcd34d;background:rgba(245,158,11,.12)}.ap-chip.pixellions{color:#c4b5fd;background:rgba(168,85,247,.12)}.ap-chip.tla_locks{color:#6ee7b7;background:rgba(16,185,129,.12)}',
    '.ap-more{padding:.4em .6em;color:var(--t-accent,#67e8f9);font-size:.72em;background:transparent;border:0;cursor:pointer;font-family:inherit}',
    ...
    '.ap-open:hover{color:var(--t-accent,#67e8f9);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent)}',
    ...
    '.ap-copy:hover{color:var(--t-accent,#67e8f9);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent)}',
    ...
    '.ap-panel{width:min(46em,100%);max-height:calc(100vh - 5em);display:flex;flex-direction:column;background:#0f1117;border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 30%,transparent);border-radius:1em;box-shadow:0 30px 60px rgba(0,0,0,.6);font-family:Outfit,system-ui,sans-serif;color:#e5e7eb}',
    ...
    '.ap-tab.on{background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 15%,transparent);color:var(--t-accent,#67e8f9);border-color:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 60%,transparent)}',
    ...
    '.ap-sort.on{color:var(--t-accent,#67e8f9);border-color:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 40%,transparent);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 10%,transparent)}',
    '.ap-list{overflow:auto;padding:.25em .6em .6em;flex:1}',
    '.ap-list .ap-row{padding:.5em .6em}',
    '.ap-row .ap-met{font-family:"JetBrains Mono",ui-monospace,Menlo,monospace;font-size:.72em;color:var(--t-accent,#67e8f9);flex:0 0 auto;min-width:5.5em;text-align:right}',
    ...
    '.ap-tcard{background:#0f1117;border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 40%,transparent);border-radius:.9em;padding:1.1em 1.25em;width:min(22em,92vw);box-shadow:0 20px 40px rgba(0,0,0,.6);font-family:Outfit,system-ui,sans-serif}',
    ...
    '.ap-tcard code{display:block;font-family:"JetBrains Mono",ui-monospace,Menlo,monospace;font-size:.66em;color:var(--t-accent,#67e8f9);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 8%,transparent);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 20%,transparent);border-radius:.5em;padding:.5em .6em;word-break:break-all;margin-bottom:.8em}',
    ...
    '.ap-tcard button:hover{border-color:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 60%,transparent);color:var(--t-accent,#67e8f9)}',
  ...
  function render() {
    ...
    if (state.sel && doc.activeElement !== e.input) {
      // Owner spec 2026-08-23: show the FULL address next to the name on desktop
      // ("DeFi_Patriot - terra1…") and "Name - …7ulw" on mobile — the address is
      // the identity; the name is the label.
      var isMobile = (root.innerWidth || 1024) < 768;
      var addrBit = isMobile ? ('\u2026' + state.sel.slice(-4)) : state.sel;
      e.input.value = state.name ? (state.name + ' - ' + addrBit) : (isMobile ? shortAddr(state.sel) : state.sel);
      e.input.classList.toggle('mono', !state.name);
      e.input.title = state.sel;
      var rr = state.byAddr[state.sel]; e.input.style.color = rr && rr.ent ? '#fcd34d' : '';
    }
    ...
    if (e.copy) { e.copy.style.display = state.sel ? '' : 'none'; if (!e.copy.title) e.copy.title = 'Copy the selected address'; }
  ...
  function mount(opts) {
    ...
    host.innerHTML = '<div class="ap"><div class="ap-pill"><i class="fas fa-magnifying-glass"></i>' +
      ...
      '<button type="button" class="ap-copy" aria-label="Copy the selected address"><i class="fas fa-copy"></i></button>' +
    ...
    var e = state.els = { root: host.firstChild, pill: host.querySelector('.ap-pill'), input: host.querySelector('.ap-in'), x: host.querySelector('.ap-x'), save: host.querySelector('.ap-save'), copy: host.querySelector('.ap-copy'), open: host.querySelector('.ap-open'), menu: host.querySelector('.ap-menu') };
    ...
    // Copy the selected address straight from the pill (2026-08-23) — no need to
    // open the full panel. Feedback verifies WHAT was copied: icon flips to a
    // check and the title shows the short address for 1.6s.
    if (e.copy) e.copy.addEventListener('click', function (ev) {
      ev.stopPropagation();
      if (!state.sel) return;
      var w2 = state.sel;
      var done = function () {
        e.copy.innerHTML = '<i class="fas fa-check"></i>'; e.copy.classList.add('ok');
        e.copy.title = 'Copied ' + shortAddr(w2);
        setTimeout(function () { e.copy.innerHTML = '<i class="fas fa-copy"></i>'; e.copy.classList.remove('ok'); e.copy.title = 'Copy the selected address'; }, 1600);
      };
      try { navigator.clipboard.writeText(w2).then(done, done); } catch (er) { done(); }
    });
