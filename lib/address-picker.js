...
(function (root, doc) {
  ...
  var CSS = [
    ...
    '.ap-open{flex:0 0 auto;width:1.6rem;height:1.6rem;border-radius:999px;border:0;background:transparent;color:#94a3b8;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:.75rem}',
    '.ap-open:hover{color:#67e8f9;background:rgba(34,211,238,.12)}',
    '.ap-row .ap-full{display:flex;align-items:center;gap:.35rem;min-width:0}',
    '.ap-row .ap-addr{font-family:"JetBrains Mono",ui-monospace,Menlo,monospace;font-size:.62rem;color:#64748b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
    '.ap-copy{flex:0 0 auto;width:1.3rem;height:1.3rem;border-radius:.35rem;border:0;background:transparent;color:#64748b;cursor:pointer;font-size:.65rem;display:inline-flex;align-items:center;justify-content:center}',
    '.ap-copy:hover{color:#67e8f9;background:rgba(34,211,238,.12)}',
    '.ap-menu.wide{width:min(34rem,94vw)}',
    '.ap-ov{position:fixed;inset:0;background:rgba(0,0,0,.65);backdrop-filter:blur(4px);z-index:9100;display:none;align-items:flex-start;justify-content:center;padding:4rem 1rem 1rem}',
    '.ap-ov.open{display:flex}',
    '.ap-panel{width:min(46rem,100%);max-height:calc(100vh - 5rem);display:flex;flex-direction:column;background:#0f1117;border:1px solid rgba(34,211,238,.3);border-radius:1rem;box-shadow:0 30px 60px rgba(0,0,0,.6);font-family:Outfit,system-ui,sans-serif;color:#e5e7eb}',
    '.ap-ph{display:flex;align-items:center;gap:.6rem;padding:.8rem 1rem;border-bottom:1px solid rgba(255,255,255,.06)}',
    '.ap-ph .ap-pill{flex:1;max-width:none;min-width:0}',
    '.ap-ph .ap-close{width:2rem;height:2rem;border-radius:999px;border:0;background:rgba(255,255,255,.06);color:#94a3b8;cursor:pointer;font-size:.9rem}',
    '.ap-ph .ap-close:hover{color:#fff}',
    '.ap-tabs{display:flex;gap:.35rem;padding:.6rem 1rem .2rem;flex-wrap:wrap}',
    '.ap-tab{font-size:.72rem;font-weight:600;padding:.3rem .65rem;border-radius:999px;border:1px solid #374151;background:rgba(31,41,55,.5);color:#cbd5e1;cursor:pointer;font-family:inherit}',
    '.ap-tab.on{background:rgba(34,211,238,.15);color:#67e8f9;border-color:rgba(34,211,238,.6)}',
    '.ap-tab small{color:#64748b;font-weight:500;margin-left:.3rem}',
    '.ap-sorts{display:flex;gap:.35rem;padding:.35rem 1rem .6rem;flex-wrap:wrap;align-items:center}',
    '.ap-sorts .lbl{font-size:.65rem;color:#64748b;text-transform:uppercase;letter-spacing:.06em;margin-right:.2rem}',
    '.ap-sort{font-size:.68rem;padding:.25rem .55rem;border-radius:.4rem;border:1px solid transparent;background:rgba(255,255,255,.04);color:#94a3b8;cursor:pointer;font-family:inherit}',
    '.ap-sort.on{color:#67e8f9;border-color:rgba(34,211,238,.4);background:rgba(34,211,238,.1)}',
    '.ap-list{overflow:auto;padding:.25rem .6rem .6rem;flex:1}',
    '.ap-list .ap-row{padding:.5rem .6rem}',
    '.ap-row .ap-met{font-family:"JetBrains Mono",ui-monospace,Menlo,monospace;font-size:.72rem;color:#67e8f9;flex:0 0 auto;min-width:5.5rem;text-align:right}',
    '.ap-empty{padding:1rem;color:#64748b;font-size:.8rem;text-align:center}',
    ...
    '.ap-tcard{background:#0f1117;border:1px solid rgba(34,211,238,.4);border-radius:.9rem;padding:1.1rem 1.25rem;width:min(22rem,92vw);box-shadow:0 20px 40px rgba(0,0,0,.6);font-family:Outfit,system-ui,sans-serif}',
    '.ap-tcard h3{margin:0 0 .25rem;color:#fff;font-size:1.05rem;font-weight:700}',
    '.ap-tcard p{margin:0 0 .6rem;color:#94a3b8;font-size:.78rem}',
    '.ap-tcard code{display:block;font-family:"JetBrains Mono",ui-monospace,Menlo,monospace;font-size:.66rem;color:#67e8f9;background:rgba(34,211,238,.08);border:1px solid rgba(34,211,238,.2);border-radius:.5rem;padding:.5rem .6rem;word-break:break-all;margin-bottom:.8rem}',
    '.ap-tcard button{width:100%;padding:.55rem;border-radius:.6rem;border:1px solid #374151;background:rgba(31,41,55,.7);color:#e5e7eb;font-weight:600;cursor:pointer;font-family:inherit}',
    '.ap-tcard button:hover{border-color:rgba(34,211,238,.6);color:#67e8f9}',
    '@media(max-width:767px){.ap-pill{min-width:0;max-width:11rem}.ap-ov{padding:3.5rem .5rem .5rem}}',
  ].join('\n');
  ...
  function buildRoster(cat) {
    ...
    Object.keys(ba).forEach(function (addr) {
      ...
      out.push({ wallet: addr, name: v.handle || null, groups: groups, m: m, nameLower: (v.handle || '').toLowerCase() });
    });
  ...
  function loadPositions() {
    ...
    state.posLoading = fetch(POSITIONS_URL, { cache: 'no-store' }).then(function (r) { return r.json(); }).then(function (pos) {
      ...
      (pos.members || []).forEach(add); if (pos.treasury) add(pos.treasury); state.posLoaded = true;
  ...
  function place() {
    ...
    var w = Math.min(544, vw * 0.94), left = Math.max(8, Math.min(r.right - w, vw - w - 8));
  ...
  function rowHtml(r, hi, q, metric) {
    return '<button type="button" class="ap-row' + (hi ? ' hi' : '') + '" data-w="' + esc(r.wallet) + '">' +
  ...
  root.AddressPicker = {
    mount: mount, select: function (w) { return select(w); }, set: function (w) { return select(w); }, clear: clear, openPanel: openPanel, closePanel: closePanel, copy: copyAddr,
