/* =============================================================================
 * lib/cron-registry.js — ONE source of truth for platform health.
 * -----------------------------------------------------------------------------
 * WHY THIS EXISTS (2026-08-12): index.html and tla-stats.html each carried
 * their own hardcoded CRON_REGISTRY, and transparency-hub read a THIRD source
 * (the dead legacy system-health file). They drifted: tla-stats showed every
 * job red because its cadences still said "weekly" for jobs that now run
 * hourly, the hub showed 42% confidence off a frozen file, and neither knew
 * about products added during the org migration (participants, dao-dashboard,
 * tla-flows, token-catalog, catalog, nft-analytics).
 *
 * Every page now reads THIS file. Adding a cron = one entry here, and it
 * appears everywhere.
 *
 * CADENCE HONESTY: `cadenceMs` is what the Render schedule actually is, not an
 * aspiration. `warningMs` / `staleMs` are derived (2x / 4x by default) so a
 * single late run never cries wolf, but a genuinely dead job goes red fast.
 * Anything measured 2026-08-12 against live heartbeat ages.
 * 2026-09-10 foundations table: tla-voting is HOURLY (was judged 15-min → false 'late' every hour); nft-flows is
 * 15-MIN (was judged daily → a dead walker took 2 days to go red); added eris-apr, credia, member-census.
 * 2026-09-12 (late) aDAO migration: nft-inventory / nft-analytics / adao-daily heartbeats now live in nft-collections/adao/.
 * 2026-09-12 NFT ledger milestone: the three per-collection ledger crons (org-nft-flows-<slug>, hourly, publishing
 * into thealliancedao/nft-collections/<slug>/) registered — two of them ran for a day under the wrong slug and no
 * registry noticed. The aDAO daily state-diff job is now named org-nft-adao-daily (was org-nft-flows).
 * 2026-09-14 fresh-but-failed: a heartbeat whose own status is 'failed' (what nft-flows writes on FATAL) was not treated
 * as producer-broken — org-nft-flows-tla-locks failed every run for 13 h on 2026-09-13 and rendered green here. Same
 * rule as org-system-health 1.0.7: failed / error / stuck outrank a fresh timestamp.
 * 2026-09-14 (B.5): price-history writer heartbeat registered — org-token-catalog appends the daily price row and now
 * writes price-history/heartbeat.json with its own status (was the July backfill's fossil, watched by nobody).
 * 2026-09-21 Lion DAO positions: org-ally-positions-liondao registered (hourly :20, heartbeat in dao-originations/lion-dao/positions/).
 * 2026-09-22 Lion DAO holders: ally-holders-liondao registered — a duty inside org-ally-positions-liondao (≥20 h cadence), heartbeat dao-originations/lion-dao/holders-heartbeat.json.
 * ============================================================================= */
(function (root) {
  'use strict';

  var CORE = 'https://raw.githubusercontent.com/thealliancedao/tla-core/main';
  var NFTC = 'https://raw.githubusercontent.com/thealliancedao/nft-collections/main';   // per-collection ledgers live here

  // Cadence presets — keep the thresholds in ONE place so every product with
  // the same schedule is judged identically.
  var HOURLY   = { cadence: 'hourly',   cadenceMs: 36e5,      warningMs: 2 * 36e5,  staleMs: 4 * 36e5 };
  var FAST     = { cadence: '15 min',   cadenceMs: 9e5,       warningMs: 36e5,      staleMs: 3 * 36e5 };
  var SIXHOUR  = { cadence: 'every 6h', cadenceMs: 6 * 36e5,  warningMs: 9 * 36e5,  staleMs: 18 * 36e5 };
  var DAILY    = { cadence: 'daily',    cadenceMs: 864e5,     warningMs: 30 * 36e5, staleMs: 48 * 36e5 };

  /* Each entry:
   *   key         stable id (used by pages for lookup)
   *   label       human name
   *   group       'core' | 'dex' | 'nft' | 'governance' | 'meta'
   *   description what it powers — shown under the label
   *   sourceUrl   heartbeat (or the product itself when it carries a timestamp)
   *   tsPath      where the timestamp lives, dot-notation; first hit wins
   *   job         the Render job that writes it — so a red dot tells you WHERE to look
   *   ...cadence  spread from a preset above
   */
  var CRONS = [
    // ---- core aggregation (org-member-data, hourly) -------------------------
    Object.assign({
      key: 'tla-snapshot', label: 'TLA Snapshot', group: 'core',
      description: 'Unified aggregator — pools, buckets, VP totals, rewards model',
      sourceUrl: CORE + '/member-data/tla-snapshot/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-member-data',
    }, HOURLY),
    Object.assign({
      key: 'tla-participants', label: 'TLA Participants', group: 'core',
      description: 'Every veLUNA lock holder — names, locks, LPs, pending rewards',
      sourceUrl: CORE + '/member-data/participants/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-member-data',
    }, HOURLY),
    Object.assign({
      key: 'adao-positions', label: 'aDAO Positions', group: 'core',
      description: 'Treasury, council + all aDAO member portfolios',
      sourceUrl: CORE + '/member-data/positions/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-member-data',
    }, HOURLY),
    Object.assign({
      key: 'member-census', label: 'Member census (daily)', group: 'core',
      description: 'Daily member snapshot + rollups (02:45 UTC) — index, member-portfolio, tla-stats history',
      sourceUrl: CORE + '/member-data/snapshots/heartbeat.json',
      tsPath: ['generated_at', 'capturedAt'], job: 'org-member-data',
    }, DAILY),
    Object.assign({
      key: 'dao-dashboard', label: 'DAO Dashboard', group: 'core',
      description: 'Treasury value, TLA deposits, unclaimed rewards, rebase, alliances',
      sourceUrl: CORE + '/member-data/dao-dashboard/current.json',
      tsPath: ['meta.generated_at'], job: 'org-member-data',
    }, HOURLY),
    Object.assign({
      key: 'network-and-prices', label: 'Network & Prices', group: 'core',
      description: 'LUNA price, LST hub ratios, token prices — priced by every other job',
      sourceUrl: CORE + '/network-and-prices/heartbeat.json',
      tsPath: ['capturedAt'], job: 'nap-org',
    }, HOURLY),

    // ---- governance / voting ------------------------------------------------
    Object.assign({
      key: 'tla-voting', label: 'Voting & Bribes', group: 'governance',
      description: 'Votes, locks, bribes, distributions — continuous chain capture',
      sourceUrl: CORE + '/tla-voting/events/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-tla-voting',
    }, HOURLY),
    Object.assign({
      key: 'tla-flows', label: 'TLA Flows', group: 'governance',
      description: 'LP deposit/withdraw flow events (block walker)',
      sourceUrl: CORE + '/tla-flows/events/heartbeat.json',
      tsPath: ['capturedAt', 'generated_at'], job: 'org-tla-flows',
    }, FAST),
    Object.assign({
      key: 'dao-governance', label: 'DAO Governance', group: 'governance',
      description: 'Proposals, votes and trust-scored messages per DAO',
      sourceUrl: 'https://raw.githubusercontent.com/thealliancedao/dao-originations/main/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-dao-governance',
    }, SIXHOUR),
    Object.assign({
      // 2026-09-21 Lion DAO positions: the tenant-keyed positions engine (platform-crons/ally-positions, one Render
      // service per ally). Heartbeat carries its own status + errors count; fresh-but-failed rule applies.
      key: 'ally-positions-liondao', label: 'Lion DAO positions', group: 'governance',
      description: 'Lion DAO roster wallets: TLA, locks, balances, delegations, validator commission — reconciled vs phoenix.money every run (hourly)',
      sourceUrl: 'https://raw.githubusercontent.com/thealliancedao/dao-originations/main/lion-dao/positions/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-ally-positions-liondao',
    }, HOURLY),
    Object.assign({
      // 2026-09-22 Lion DAO holders: ally-positions/holders.js — a duty of org-ally-positions-liondao (index.js 1.2.2 runs it when its heartbeat is ≥20 h old); no service of its own. pyROAR = every cw20 account
      // walked + summed against total_supply; ROAR20 = every token account via Helius DAS folded to owners. Status in the heartbeat.
      key: 'ally-holders-liondao', label: 'Lion DAO holders (pyROAR · ROAR20)', group: 'governance',
      description: 'Who holds pyROAR (the frozen burn ledger) and ROAR20 (Solana), every account, with the supply gate Δ — daily, run inside the positions job when ≥20 h old',
      sourceUrl: 'https://raw.githubusercontent.com/thealliancedao/dao-originations/main/lion-dao/holders-heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-ally-positions-liondao',
    }, DAILY),

    // ---- DEX ----------------------------------------------------------------
    Object.assign({
      key: 'astroport', label: 'Astroport Pools', group: 'dex',
      description: 'Pool TVL, volume, epoch aggregates (feeds TLA Snapshot)',
      sourceUrl: CORE + '/dex-data/astroport/epochs/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-dex-data',
    }, HOURLY),
    Object.assign({
      key: 'skeletonswap', label: 'Skeleton Swap', group: 'dex',
      description: 'SS pool reserves + TVL, chain-direct since the warlock freeze',
      sourceUrl: CORE + '/dex-data/skeletonswap/rolling/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-dex-data',
    }, HOURLY),
    // 2026-09-10 foundations table: products pages read that had NO entry (a dead eris-apr/credia module inside
    // org-dex-data was invisible — the astroport epochs heartbeat stayed green).
    Object.assign({
      key: 'eris-apr', label: 'Eris APR (per pool)', group: 'dex',
      description: 'Eris-convention APR/APY per gauge pool — new-here-tla, tla-stats Pools',
      sourceUrl: CORE + '/dex-data/eris-apr/heartbeat.json',
      tsPath: ['generated_at', 'capturedAt'], job: 'org-dex-data',
    }, HOURLY),
    Object.assign({
      key: 'credia', label: 'Credia markets', group: 'dex',
      description: 'Credia lending-market snapshot (wBTC.creda.a gauge) + rate history',
      sourceUrl: CORE + '/dex-data/credia/snapshots/heartbeat.json',
      tsPath: ['generated_at', 'capturedAt'], job: 'org-dex-data',
    }, HOURLY),
    Object.assign({
      key: 'votion', label: 'Votion', group: 'dex',
      description: 'Vault holdings + per-user Votion positions',
      sourceUrl: CORE + '/votion/heartbeat.json',
      tsPath: ['capturedAt', 'positions_at'], job: 'org-votion',
    }, HOURLY),

    // ---- NFT ----------------------------------------------------------------
    Object.assign({
      key: 'nft-inventory', label: 'NFT Inventory', group: 'nft',
      description: 'Per-NFT ownership, staking, listings, backing (10k tokens)',
      sourceUrl: NFTC + '/adao/snapshots/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-nft-inventory',
    }, FAST),
    Object.assign({
      key: 'nft-analytics', label: 'NFT Analytics', group: 'nft',
      description: 'Floors by rarity, sales analytics, explorer feed',
      sourceUrl: NFTC + '/adao/snapshots/analytics-heartbeat.json',
      tsPath: ['generated_at', 'capturedAt'], job: 'org-nft-inventory',
    }, DAILY),

    Object.assign({
      key: 'nft-flows', label: 'aDAO daily state-diff', group: 'nft',
      description: 'Per-token transfer/mint/sale flow ledger by month (daily state diff of nfts.json)',
      sourceUrl: NFTC + '/adao/flows/heartbeat.json',
      tsPath: ['capturedAt', 'generated_at'], job: 'org-nft-adao-daily',
    }, FAST),

    // ---- per-collection on-chain ledgers (nft-collections repo, one Render service each, hourly) ----------
    Object.assign({
      key: 'nft-ledger-adao', label: 'aDAO ledger (on-chain)', group: 'nft',
      description: 'Mints, sales, listings, stakes, breaks — nft-collections/adao/ledger (block cursor, hourly)',
      sourceUrl: NFTC + '/adao/nft-flows/heartbeat.json',
      tsPath: ['ran_at'], job: 'org-nft-flows-adao',
    }, HOURLY),
    Object.assign({
      key: 'nft-ledger-pixel-lions', label: 'Pixel Lions ledger', group: 'nft',
      description: 'Pixel Lions mints, sales, listings, stakes — nft-collections/pixel-lions/ledger (hourly)',
      sourceUrl: NFTC + '/pixel-lions/nft-flows/heartbeat.json',
      tsPath: ['ran_at'], job: 'org-nft-flows-pixel-lions',
    }, HOURLY),
    Object.assign({
      key: 'nft-ledger-tla-locks', label: 'TLA Locks ledger', group: 'nft',
      description: 'Lock create/add/merge/split/migrate/withdraw + Atrium listings — nft-collections/tla-locks/ledger (hourly)',
      sourceUrl: NFTC + '/tla-locks/nft-flows/heartbeat.json',
      tsPath: ['ran_at'], job: 'org-nft-flows-tla-locks',
    }, HOURLY),

    // ---- registries / meta --------------------------------------------------
    Object.assign({
      // Self-escalating discovery cron — observed ~5h between runs, not hourly.
      // Calling it hourly made a perfectly healthy job read "RUN PENDING".
      key: 'token-catalog', label: 'Token Catalog', group: 'meta',
      description: 'Token identity + pricing coverage, keyed by address',
      sourceUrl: CORE + '/token-catalog/snapshots/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-token-catalog',
    }, SIXHOUR),
    Object.assign({
      // 2026-09-14 (B.5): the daily price-history append's writer heartbeat (status ok / failed + reason)
      key: 'price-history', label: 'Price History (daily append)', group: 'meta',
      description: 'Daily rich price row appended to price-history/YYYY/MM.json — writer heartbeat with its own status',
      sourceUrl: CORE + '/price-history/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-token-catalog',
    }, SIXHOUR),
    Object.assign({
      // 2026-08-24 (capa-supply v2): the CAPA custody map is its own product
      // (rides org-token-catalog; carries capturedAt itself, no heartbeat file).
      key: 'capa-supply', label: 'CAPA Supply Map', group: 'meta',
      description: 'Every custody form CAPA sits in, sum-guarded, + per-wallet rows',
      sourceUrl: CORE + '/token-catalog/supply/capa/current.json',
      tsPath: ['capturedAt'], job: 'org-token-catalog',
    }, SIXHOUR),
    Object.assign({
      key: 'fuel-supply', label: 'FUEL Supply Map', group: 'meta',
      description: 'FUEL on Neutron (Boost DAO stakers, treasury, escrow) + Terra IBC holders, sum-guarded',
      sourceUrl: CORE + '/token-catalog/supply/fuel/current.json',
      tsPath: ['capturedAt'], job: 'org-token-catalog',
    }, SIXHOUR),
    Object.assign({
      key: 'address-catalog', label: 'Address Catalog', group: 'meta',
      description: 'Who we track — DAO members, lockers, allies, by slug',
      sourceUrl: CORE + '/catalog/snapshots/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-address-catalog',
    }, DAILY),
    Object.assign({
      key: 'lp-grades', label: 'LP Grades', group: 'meta',
      description: 'The routing oracle — windowed pool grades per the public rubric',
      sourceUrl: CORE + '/lp-grades/snapshots/heartbeat.json',
      tsPath: ['capturedAt'], job: 'org-lp-grades',
    }, DAILY),
    Object.assign({
      key: 'system-health', label: 'System Health', group: 'meta',
      description: 'Invariant checks across every org product',
      sourceUrl: CORE + '/system-health/current.json',
      tsPath: ['meta.generated_at', 'capturedAt'], job: 'org-system-health',
    }, HOURLY),
  ];

  function dig(obj, path) {
    return path.split('.').reduce(function (o, k) { return (o == null ? o : o[k]); }, obj);
  }

  // Freshness verdict from age alone — the same rule everywhere.
  //   fresh   : within cadence
  //   due     : past cadence but under warning (a run is simply pending)
  //   warning : past warning threshold
  //   stale   : past stale threshold — something is wrong
  function verdict(entry, ageMs) {
    if (ageMs == null || !isFinite(ageMs)) return 'unknown';
    if (ageMs <= entry.cadenceMs) return 'fresh';
    if (ageMs <= entry.warningMs) return 'due';
    if (ageMs <= entry.staleMs) return 'warning';
    return 'stale';
  }

  function humanAge(ms) {
    if (ms == null || !isFinite(ms)) return '—';
    var s = ms / 1000;
    if (s < 90) return Math.round(s) + 's ago';
    if (s < 5400) return Math.round(s / 60) + 'm ago';
    if (s < 129600) return Math.round(s / 3600) + 'h ago';
    return Math.round(s / 86400) + 'd ago';
  }

  // Fetch every entry; never throws. Each result carries enough context that a
  // red dot tells you WHICH Render job to open.
  function fetchAll(opts) {
    var bust = (opts && opts.cacheBust !== false);
    return Promise.all(CRONS.map(function (c) {
      var url = c.sourceUrl + (bust ? ('?t=' + Date.now()) : '');
      return fetch(url)
        .then(function (r) { return r.ok ? r.json() : null; })
        .then(function (d) {
          if (!d) {
            return Object.assign({}, c, { state: 'unknown', ageMs: null, capturedAt: null,
              detail: 'no heartbeat published yet' });
          }
          var ts = null;
          for (var i = 0; i < c.tsPath.length && ts == null; i++) ts = dig(d, c.tsPath[i]);
          var ageMs = ts ? (Date.now() - Date.parse(ts)) : null;
          // A product may declare its own status (ok / partial / stuck); a
          // self-declared problem outranks a fresh timestamp.
          var declared = d.status || (d.meta && d.meta.status) || null;
          var state = verdict(c, ageMs);
          // FRESHNESS vs FINDINGS are different questions (fixed 2026-08-12).
          // system-health legitimately reports status:"violation" when an
          // INVARIANT fails — that is the job WORKING, and it says nothing about
          // whether the job ran on time. Conflating them made a job that runs
          // every 19 minutes render as "LATE".
          //   - 'stuck'/'error'/'failed' mean the producer itself is unhealthy → warning
          //     ('failed' added 2026-09-14 — nft-flows FATAL writes it; a fresh
          //     timestamp on a failed run is the job crashing on schedule)
          //   - 'violation'/'partial'/'degraded' are FINDINGS the product is reporting →
          //     freshness stays honest; the finding is surfaced separately.
          var producerBroken = (declared === 'stuck' || declared === 'error' || declared === 'failed');
          if (producerBroken && state === 'fresh') state = 'warning';
          var finding = (declared && declared !== 'ok' && declared !== 'pass' && !producerBroken) ? declared : null;
          return Object.assign({}, c, {
            state: state, ageMs: ageMs, capturedAt: ts, declared: declared,
            finding: finding,                       // a report FROM the job, not a fault OF the job
            ageText: humanAge(ageMs),
            detail: finding ? (humanAge(ageMs) + ' · reports "' + finding + '"') : humanAge(ageMs),
          });
        })
        .catch(function (e) {
          return Object.assign({}, c, { state: 'unknown', ageMs: null, capturedAt: null,
            detail: 'unreachable: ' + (e && e.message ? e.message : 'error') });
        });
    }));
  }

  // Overall verdict + a confidence number pages can render identically.
  function summarize(results) {
    var counts = { fresh: 0, due: 0, warning: 0, stale: 0, unknown: 0 };
    results.forEach(function (r) { counts[r.state] = (counts[r.state] || 0) + 1; });
    var healthy = counts.fresh + counts.due;          // "due" is normal, not a fault
    var scored = healthy + counts.warning + counts.stale;
    var confidence = scored ? Math.round((healthy / scored) * 100) : 100;
    var overall = counts.stale ? 'degraded' : (counts.warning ? 'watch' : 'ok');
    return {
      counts: counts, confidence: confidence, overall: overall,
      // Attention = jobs that are actually unhealthy. A job reporting a
      // FINDING (e.g. system-health flagging an invariant) is doing its job;
      // it is listed separately so the number stays meaningful.
      attention: results.filter(function (r) { return r.state === 'warning' || r.state === 'stale' || r.state === 'unknown'; }),
      findings: results.filter(function (r) { return r.finding; }),
      newestAt: results.reduce(function (a, r) {
        return (r.capturedAt && (!a || r.capturedAt > a)) ? r.capturedAt : a;
      }, null),
    };
  }

  root.CronRegistry = {
    CRONS: CRONS,
    GROUPS: ['core', 'governance', 'dex', 'nft', 'meta'],
    GROUP_LABELS: { core: 'Core data', governance: 'Governance', dex: 'DEX', nft: 'NFT', meta: 'Registries' },
    fetchAll: fetchAll,
    summarize: summarize,
    humanAge: humanAge,
    verdict: verdict,
    STATE_LABELS: {
      fresh: 'Fresh', due: 'Run pending', warning: 'Late',
      stale: 'Stale', unknown: 'No signal',
    },
    // Shared colours so every page's dots mean the same thing.
    STATE_COLORS: {
      fresh: '#22c55e', due: '#84cc16', warning: '#f59e0b',
      stale: '#ef4444', unknown: '#6b7280',
    },
  };
})(typeof window !== 'undefined' ? window : globalThis);
