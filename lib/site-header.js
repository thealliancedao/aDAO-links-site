 ...
 *
 * The header carries an empty slot, `#sh-picker`, left of the globe. Step 2
 * (lib/address-picker.js) mounts the global address picker there; until then
 * the slot renders nothing and takes no space.
 *
 * USAGE:
...
(function (root, doc) {
  ...
  var CSS = [
    '#site-header{font-family:Outfit,system-ui,sans-serif;background:rgba(10,11,15,.92);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border-bottom:1px solid rgba(255,255,255,.06);position:relative;z-index:45}',
  ...
  function render(opts) {
    ...
    return '<div class="sh-wrap">' +
      ...
      '<div class="sh-right"><div id="sh-picker"></div><span class="sh-globe"><img src="/assets/planets/Terra.PNG" alt=""></span></div>' +
      '</div>' + mtabs;
  }
