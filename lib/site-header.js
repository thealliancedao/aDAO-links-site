...
(function (root, doc) {
  ...
  function applyTenantTheme(t, ctx) {
    ...
    var css = ':root{--ally-accent:' + accent + ';--ally-accent2:' + accent2 + ';--ally-bg:' + bg + ';--ally-surface:' + surface + ';--ally-text:' + text + ';--ally-font:' + font + '}' +
    ...
    if (th.style === 'pixel') {
      css += sel + ' .rounded-xl,' + sel + ' .rounded-lg,' + sel + ' .rounded-md,' + sel + ' .rounded,' + sel + ' .nft-card,' + sel + ' .form-select,' + sel + ' .form-input,' + sel + ' .multi-select-button,' + sel + ' .multi-select-dropdown,' + sel + ' .sh-tab,' + sel + ' .sh-tenant,' + sel + ' button{border-radius:0!important}' +
        ...
        sel + ' .sh-tilewrap,' + sel + ' .sh-subtab-wrap{border:2px solid ' + accent + '!important;box-shadow:6px 6px 0 ' + accent + '}' +
    ...
    try { prefsSet({ theme_cache: { slug: t.slug, css: css, logo: t.logo || null, hide: (t.nav && t.nav.hide) || [], new_here: t.nav ? t.nav.new_here : undefined, font: family !== 'Inter' ? family : null } }); } catch (e) {}
  }
  ...
  function mount(opts) {
    ...
    if (pre) { try { var img0 = el.querySelector('.sh-logo img'); if (img0 && pre.logo) img0.src = pre.logo; el.setAttribute('data-sh-tenant', pre.slug); (pre.hide || []).forEach(function (pg) { el.querySelectorAll('.sh-tab[data-page="' + pg + '"], .sh-btab[data-page="' + pg + '"]').forEach(function (x) { x.remove(); }); }); } catch (e) {} }
