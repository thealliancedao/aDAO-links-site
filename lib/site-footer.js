 ...
 * v3.7 (2026-09-20, owner): a second way to support — a gift to an ally treasury (aDAO · Lion DAO · Pixel Lions DAO) with its own memo, every target from tla-core/docs/curated/supporters.json, tracked and showcased beside the builder's on the Supporters page; v3.6 (2026-08-26): help drawer shows "What others asked" from the agent's privacy-preserving question log; v3.5: 'New here?' link; v3.4: builder's note + copyable address; ecosystem logo banner + builder contact; v3.3: +Creda, Atrium/Votion logos; v3.1: honest legal row.
 * -----------------------------------------------------------------------------
...
(function (root, doc) {
      ...
      '<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-6">' +
        ...
        '<div class="grid grid-cols-2 md:grid-cols-12 gap-8 text-sm text-left">' +
          ...
          '<div class="md:col-span-4">' + colH('This project') +
            ...
            '<a href="install.html" class="sf-prow" title="Add the Ally app to your home screen"><i class="fas fa-mobile-alt sf-pico"></i>Get the app</a>' +   // 2026-09-11: every page, real link (was a Home-only button opening an old modal)
          '</div>' +
        '</div>' +
        ...
        '<div class="mt-6 pt-5 border-t border-gray-800/70 text-center">' +
          ...
          '</div>' +
          // v3.7 (owner): the second option — give to an ally instead; targets + memos from the curated registry (filled at mount)
          '<div class="text-[13px] text-gray-300 mt-4">Or support an <span class="text-amber-300 font-semibold">ally community</span> directly: a gift to a DAO treasury with its memo is tracked the same way and showcased on the <a href="supporters.html#allies" class="text-cyan-300 hover:underline font-semibold">Supporters page</a>.</div>' +
          '<div id="sf-ally-targets" class="mt-2 flex items-center justify-center gap-2 flex-wrap text-[12px]"><span class="text-gray-600">loading the registry…</span></div>' +
        '</div>' +
  ...
  function ensureStyle() {
    ...
    st.id = 'sf-style';
    st.textContent =
      '.sf-prow{display:flex;align-items:center;gap:.5em;padding:.15em 0;color:#9ca3af;font-size:.875rem;line-height:1.4;transition:color .15s;text-decoration:none}.sf-prow:hover{color:var(--t-accent,#67e8f9)}' +
      '.sf-pico{width:1.1em;text-align:center;font-size:.8em;color:#4b5563;flex:0 0 auto}.sf-prow:hover .sf-pico{color:var(--t-accent,#67e8f9)}' +
      ...
      '.sf-eco:hover{color:#e5e7eb;border-color:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 55%,transparent);transform:translateY(-1px)}' +
      ...
      '.sf-eco .sf-eco-mark{display:inline-flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:800;color:var(--t-accent,#67e8f9);background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent)}' +
      ...
      '.sf-footer .sf-meta a:hover{color:var(--t-accent,#22d3ee)}' +
      ...
      '.cron-health-trigger:hover{color:var(--t-accent,#22d3ee)}' +
  ...
  function wireHealth() {
    ...
    healthResults().then(function (results) {
      ...
      // v3.7: ally targets from the curated registry — one chip per treasury: label · memo · copy the address
      (function () { var box = doc.getElementById('sf-ally-targets'); if (!box || !root.fetch) return;
        root.fetch('https://raw.githubusercontent.com/thealliancedao/tla-core/main/docs/curated/supporters.json?t=' + Math.floor(Date.now() / 9e5)).then(function (r) { return r.ok ? r.json() : null; }).then(function (reg) {
          var ts = (reg && reg.targets || []).filter(function (t) { return t && t.kind === 'ally' && t.address && t.memo; });
          if (!ts.length) { box.innerHTML = '<span class="text-gray-600">no ally targets listed yet</span>'; return; }
          box.innerHTML = ts.map(function (t, i) { return '<span class="inline-flex items-center gap-1.5 bg-white/[0.04] rounded px-2.5 py-1"><span class="text-gray-300">' + t.label.replace(/</g, '&lt;') + '</span><span class="text-gray-600">·</span><code class="mono text-amber-300">' + t.memo.replace(/</g, '&lt;') + '</code><button type="button" class="sf-ally-copy text-gray-400 hover:text-cyan-300" data-addr="' + t.address + '" title="copy ' + t.address + '"><i class="fas fa-copy"></i></button></span>'; }).join('');
          Array.prototype.forEach.call(box.querySelectorAll('.sf-ally-copy'), function (b) { b.addEventListener('click', function () { var t = b.getAttribute('data-addr'); (navigator.clipboard ? navigator.clipboard.writeText(t) : Promise.reject()).then(function () { b.innerHTML = '<i class="fas fa-check"></i>'; setTimeout(function () { b.innerHTML = '<i class="fas fa-copy"></i>'; }, 1500); }, function () { root.prompt('copy the address', t); }); }); });
        }).catch(function () { box.innerHTML = '<span class="text-gray-600">registry unavailable</span>'; }); })();
  ...
  function ensureHelpBubble() {
    ...
    a.setAttribute('style',
      ...
      'padding:10px 14px;border-radius:9999px;background:rgba(13,17,23,.92);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 35%,transparent);' +
      'color:var(--t-accent,#67e8f9);font-weight:600;font-size:13px;text-decoration:none;box-shadow:0 4px 20px rgba(0,0,0,.5), 0 0 12px color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent);' +
      'backdrop-filter:blur(8px);transition:transform .15s, box-shadow .15s;');
    a.onmouseenter = function () { a.style.transform = 'scale(1.06)'; a.style.boxShadow = '0 6px 24px rgba(0,0,0,.55), 0 0 18px color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 25%,transparent)'; };
    a.onmouseleave = function () { a.style.transform = ''; a.style.boxShadow = '0 4px 20px rgba(0,0,0,.5), 0 0 12px color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 12%,transparent)'; };
  ...
  function drawerHtml() {
    return '' +
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,.08)">' +
        '<div style="font-weight:700;font-size:14px;color:#e5e7eb"><i class="fas fa-circle-question" style="color:var(--t-accent,#22d3ee);margin-right:8px"></i>Help — ask about this page</div>' +
        '<div><a href="address-catalog.html" title="Every address this site knows, and how it knows it" style="font-size:11px;color:var(--t-accent,#67e8f9);text-decoration:none;margin-right:12px"><i class="fas fa-address-book" style="margin-right:4px"></i>Address catalog</a>' +
        '<a href="help.html" style="font-size:11px;color:var(--t-accent,#67e8f9);text-decoration:none;margin-right:12px">Full help ↗</a>' +
      ...
      '<div style="padding:10px 12px;border-top:1px solid rgba(255,255,255,.08);display:flex;gap:8px;align-items:flex-end">' +
        ...
        '<button id="hd-send" aria-label="Send" style="background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 15%,transparent);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 30%,transparent);color:var(--t-accent,#67e8f9);border-radius:10px;min-width:44px;min-height:44px;font-size:14px;cursor:pointer;flex:0 0 auto"><i class="fas fa-paper-plane"></i></button>' +
      '</div>' +
  ...
  function renderWalletRow() {
    ...
    if (walletPinned) {
      ...
      row.innerHTML = '<span style="display:inline-flex;align-items:center;gap:6px;background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 10%,transparent);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 30%,transparent);border-radius:9999px;padding:4px 10px;font-size:11px;color:var(--t-accent,#67e8f9)">' +
      ...
      row.innerHTML = '<button type="button" id="hd-wallet-pick" style="display:inline-flex;align-items:center;gap:8px;background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 8%,transparent);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 30%,transparent);border-radius:9999px;padding:6px 12px;font-size:11px;color:var(--t-accent,#67e8f9);cursor:pointer;font-family:inherit"><i class="fas fa-user-magnifying-glass"></i>Choose an address — answers will cater to it</button>' +
  ...
  function fmtAnswer(el, raw) {
    ...
    t = t.replace(/\[([^\]]{1,80})\]\((https:\/\/[^\s)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener" style="color:var(--t-accent,#67e8f9);text-decoration:underline">$1</a>');
    ...
    t = t.replace(/\b(terra1[a-z0-9]{38,58})\b(?![^<]*<\/a>)/g, function (m) {
      return '<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);border-radius:6px;padding:1px 6px;font-family:monospace;font-size:.85em;word-break:break-all">' + m +
        ...
        ' <a href="https://chainsco.pe/terra2/address/' + m + '" target="_blank" rel="noopener" title="View on chain" style="color:var(--t-accent,#67e8f9)"><i class="fas fa-arrow-up-right-from-square"></i></a>' +
    ...
    t = t.replace(/\b([A-F0-9]{64})\b(?![^<]*<\/a>)/g, function (m) {
      return '<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);border-radius:6px;padding:1px 6px;font-family:monospace;font-size:.85em;word-break:break-all">' + m.slice(0, 10) + '…' + m.slice(-6) +
        ...
        ' <a href="https://chainsco.pe/terra2/tx/' + m + '" target="_blank" rel="noopener" title="View tx" style="color:var(--t-accent,#67e8f9)"><i class="fas fa-arrow-up-right-from-square"></i></a></span>';
    });
    t = t.replace(/(^|\n)\s*[-*]\s+/g, '$1&bull; ');
    t = t.replace(/\n/g, '<br>');
    el.innerHTML = t;
  ...
  // "What others asked" (v3.6, 2026-08-26 owner ask): the help agent already keeps a
  // privacy-preserving question log in tla-core (help-agent/questions/<yyyy-mm>.json —
  // time, page, redacted question; never IP, wallet or answer). Show the last real
  // questions (mode 'chat'; the site's own audit prompts are skipped) as a starter
  // block; tapping one puts it in the composer. Reads this month + last month.
  var OTHERS_RAW = 'https://raw.githubusercontent.com/thealliancedao/tla-core/main/help-agent/questions/';
  function esc(x) { return String(x == null ? '' : x).replace(/[&<>"]/g, function (c) { return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]; }); }
  function relTime(iso) { var ms = Date.now() - Date.parse(iso); if (!(ms >= 0)) return ''; var m = Math.round(ms / 60000); if (m < 60) return m + ' min ago'; var h = Math.round(m / 60); if (h < 48) return h + ' h ago'; return Math.round(h / 24) + ' d ago'; }
  function renderOthersAsked() {
    var log = doc.getElementById('hd-log'); if (!log || doc.getElementById('hd-others')) return;
    var box = doc.createElement('div'); box.id = 'hd-others';
    box.setAttribute('style', 'align-self:stretch;border:1px dashed color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 25%,transparent);border-radius:10px;padding:8px 10px;font-size:12px;color:#9ca3af');
    box.innerHTML = '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.12em;color:var(--t-accent,#67e8f9);margin-bottom:4px">What others asked</div><div id="hd-others-list">loading…</div>';
    log.appendChild(box);
    var now = new Date(), months = [], m = new Date(now.getTime()); for (var i = 0; i < 2; i++) { months.push(m.toISOString().slice(0, 7)); m.setUTCMonth(m.getUTCMonth() - 1); }
    Promise.all(months.map(function (ym) { return fetch(OTHERS_RAW + ym + '.json?t=' + Math.floor(Date.now() / 900000)).then(function (r) { return r.ok ? r.json() : null; }).catch(function () { return null; }); }))
      .then(function (docs) {
        var qs = []; docs.forEach(function (dd) { if (dd && dd.questions) qs = qs.concat(dd.questions); });
        qs = qs.filter(function (q) { return q && q.mode === 'chat' && q.question; }).sort(function (a, b) { return a.at < b.at ? 1 : -1; }).slice(0, 8);
        var list = doc.getElementById('hd-others-list'); if (!list) return;
        if (!qs.length) { list.innerHTML = '<span style="color:#6b7280">No questions yet this month — yours could be the first.</span>'; return; }
        list.innerHTML = qs.map(function (q, i) {
          var page = (q.page || '').replace(/^\//, '').replace(/\.html$/, '') || 'home';
          return '<div class="hd-other" data-i="' + i + '" style="cursor:pointer;padding:5px 0;border-top:1px solid rgba(255,255,255,.05);color:#d1d5db;line-height:1.35">' + esc(String(q.question).slice(0, 160)) + (q.question.length > 160 ? '…' : '') +
            '<div style="font-size:10px;color:#6b7280;margin-top:2px">' + relTime(q.at) + ' · on ' + esc(page) + '</div></div>';
        }).join('') + '<div style="font-size:10px;color:#4b5563;margin-top:4px">questions only — no wallets, no answers kept · tap one to ask it yourself</div>';
        var nodes = list.querySelectorAll('.hd-other');
        for (var k = 0; k < nodes.length; k++) (function (n) { n.onclick = function () { var inp = doc.getElementById('hd-input'); if (inp) { inp.value = qs[Number(n.getAttribute('data-i'))].question; inp.dispatchEvent(new Event('input')); inp.focus(); } }; })(nodes[k]);
      });
  }
  function hdMsg(txt, who) {
    ...
    d.setAttribute('style', 'border-radius:10px;padding:7px 10px;max-width:88%;' +
      (who === 'you' ? 'align-self:flex-end;background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 10%,transparent);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 20%,transparent);color:#e5e7eb'
  ...
  function showDisc(onAccept) {
    ...
    box.innerHTML = '<div id="hd-disc-s" style="max-height:130px;overflow-y:auto;border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:8px;font-size:11px;color:#9ca3af;line-height:1.5">' +
      ...
      '<button id="hd-disc-ok" disabled style="opacity:.4;background:color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 15%,transparent);border:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 30%,transparent);color:var(--t-accent,#67e8f9);border-radius:8px;padding:6px 12px;font-size:12px;cursor:not-allowed">I understand and accept</button></div>';
  ...
  function openHelpDrawer() {
    ...
    d.setAttribute('style', mobile
      ...
      : 'position:fixed;top:0;right:0;bottom:0;width:min(' + (drawerWide() ? '760px' : '400px') + ',94vw);z-index:70;display:flex;flex-direction:column;transition:transform .25s ease,width .2s ease;' +
        'background:rgba(10,12,16,.97);border-left:1px solid color-mix(in srgb,var(--t-accent,var(--t-accent,#22d3ee)) 20%,transparent);box-shadow:-8px 0 40px rgba(0,0,0,.6);' +
    ...
    if (wbtn) { if (mobile) wbtn.style.display = 'none'; wbtn.onclick = function () { var w = !drawerWide(); try { root.localStorage.setItem('tla_help_wide', w ? '1' : '0'); } catch (e) {} d.style.width = 'min(' + (w ? '760px' : '400px') + ',94vw)'; wbtn.style.color = w ? 'var(--t-accent,#67e8f9)' : '#6b7280'; }; wbtn.style.color = drawerWide() ? 'var(--t-accent,#67e8f9)' : '#6b7280'; }
    if (mobile) {
