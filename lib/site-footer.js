...
(function (root, doc) {
  ...
  function drawerHtml() {
    return '' +
      ...
      '<div style="padding:12px 16px;border-top:1px solid rgba(255,255,255,.08);display:flex;gap:8px">' +
        '<input id="hd-input" placeholder="Ask about what you\'re looking at…" style="flex:1;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);border-radius:8px;padding:8px 10px;color:#e5e7eb;font-size:13px;outline:none">' +
        '<button id="hd-send" style="background:rgba(34,211,238,.15);border:1px solid rgba(34,211,238,.3);color:#67e8f9;border-radius:8px;padding:8px 12px;font-size:13px;cursor:pointer"><i class="fas fa-paper-plane"></i></button>' +
      '</div>' +
  ...
  function hdAsk() {
    ...
    fetch(AGENT_URL + '/ask', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      .then(function (d) {
        hold.textContent = d.answer || d.error || 'No answer returned.';
      ...
      .catch(function () { hold.textContent = 'Unreachable right now (free hosting wakes in ~30-60s after idle). Try again shortly, or use the report form on the Help page.'; })
      .then(function () { chatBusy = false; });
  }

  function openHelpDrawer() {
    var ex = doc.getElementById('help-drawer');
    if (ex) { ex.style.transform = 'translateX(0)'; return; }
    var d = doc.createElement('div');
    d.id = 'help-drawer';
    ...
    (root.requestAnimationFrame || function (f) { setTimeout(f, 16); })(function () { d.style.transform = 'translateX(0)'; });
    doc.getElementById('hd-close').onclick = function () { d.style.transform = 'translateX(100%)'; };
    doc.getElementById('hd-send').onclick = hdAsk;
    doc.getElementById('hd-input').addEventListener('keydown', function (ev) { if (ev.key === 'Enter') hdAsk(); });
    renderWalletRow();
