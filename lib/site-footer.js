...
(function (root, doc) {
  ...
  // Ecosystem row (footer v2, 2026-08-23 owner spec): every venue TLA data touches.
  var ECO = [['Eris TLA','https://www.erisprotocol.com'],['Astroport','https://app.astroport.fi'],['Solid','https://solidcapa.com'],['Atrium','https://atrium.markets'],['Boost','https://www.boostdao.io'],['BBL','https://app.backbonelabs.io'],['Votion','https://votion.money'],['Terra','https://phoenix.money'],['DAODAO','https://daodao.zone']];
  ...
  function html(opts) {
    ...
    return '' +
      ...
      '<div class="sf-inner max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-gray-400">' +
        ...
        '<div class="sf-eco mt-4 flex flex-wrap justify-center items-center gap-x-4 gap-y-1 text-xs">' +
  '<span class="text-gray-600 uppercase tracking-wider mr-1">Ecosystem</span>' +
  ECO.map(function (l) { return '<a href="' + l[1] + '" target="_blank" rel="noopener noreferrer" class="text-gray-400 hover:text-cyan-300 transition-colors">' + l[0] + '</a>'; }).join('<span class="opacity-30">\u00b7</span>') +
'</div>' +
'<div class="sf-legal mt-5 mx-auto max-w-5xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 text-left text-xs text-gray-500">' +
  '<div><h4 class="font-bold text-gray-300 mb-1">Not financial advice</h4><p>Everything here is informational only. Data is provided \u201cas is\u201d with no warranty of accuracy, completeness, or fitness for any purpose. Nothing on this site is a recommendation to buy, sell, or hold anything. Do your own research; act at your own risk.</p></div>' +
  '<div><h4 class="font-bold text-gray-300 mb-1">Third-party data</h4><p>Figures are assembled from on-chain endpoints and external services outside our control. They can be delayed, incomplete, or wrong \u2014 verify against the source before acting on any number. Where data is missing we show a blank, never an estimate dressed as a fact.</p></div>' +
  '<div><h4 class="font-bold text-gray-300 mb-1">No wallet, no tracking</h4><p>This site is read-only. It never asks you to connect a wallet, create an account, or provide personal information. Built for the community, by a community member \u2014 not an official voice of any council or protocol.</p></div>' +
  '<div><h4 class="font-bold text-gray-300 mb-1">Open source, open door</h4><p>The capture code and every dataset live in the public org repo \u2014 review it, fork it, or use any part as inspiration. Built to host other NFT collections (explorer, analytics, DAO tooling): plug yours in, or take everything and run your own. <a href="https://github.com/thealliancedao" target="_blank" rel="noopener noreferrer" class="text-cyan-400 hover:text-cyan-300 underline">github.com/thealliancedao</a></p></div>' +
'</div>' +
        ...
        '<div class="sf-meta mt-4 text-xs text-gray-500">' +
          ...
          (opts.appInfo ? '<span class="sf-sep opacity-40 mx-2">\u00b7</span><button id="app-info-trigger" class="hover:text-cyan-400 transition-colors cursor-pointer"><i class="fas fa-mobile-alt text-cyan-400 text-xs"></i> App</button>' : '') +
        '</div>' +
      '</div></footer>';
  }
