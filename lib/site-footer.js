...
(function (root, doc) {
  ...
  function html(opts) {
    ...
    return '' +
      ...
      '<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-gray-400">' +
        ...
        '</div>' +
        '<div class="flex flex-wrap justify-center items-center space-x-6 text-sm">' + links + '</div>' +
        '<div class="mt-4 text-xs text-gray-500">' +
          '<a href="' + LOG_BASE + '/' + log + '" target="_blank" rel="noopener" class="hover:text-cyan-400 transition-colors">' +
            '<span class="font-mono" id="page-rev">' + rev + '</span>' +
            '<span class="opacity-60 mx-1">·</span><span>Changelog</span></a>' +
  ...
  function wireHealth() {
    ...
    root.CronRegistry.fetchAll().then(function (results) {
      ...
      if (!trigger) return;
      trigger.setAttribute('data-overall', sum.overall);
  ...
  function mount(opts) {
    ...
    if (host) { ensureStyle(); host.innerHTML = html(opts); }
    wireHealth();
  }
